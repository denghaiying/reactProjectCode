...
class BaseService extends HttpRequest {
 ...
 findById (id) {
    return new Promise((resolve, reject) => {
      super.get(`${this.url}/${encodeURIComponent(id)}`).then(
        response => {
  ...
  findAll(params) {
    return new Promise((resolve, reject) => {
      super.get({
        url: `${this.url}`,params:params}).then(res => {
        if (res.status > 300) {
          return reject(res)
        }
    ...
    return new Promise((resolve, reject) => {
      super.post({url:this.url, params,  data: { pageno, pagesize } })
        .then(response => {
          if (response.status === 200) {
           return resolve(response.data);
          } else {
  ...
  update(data) {
   ...
   delete (data) {
    ...
    return new Promise((resolve, reject) => {
      super.delete({url: `${this.url}/${encodeURIComponent(data.id)}`}).then(response => {
   ...
   findForPage(url,params, pageno = 1, pagesize = 10) {
    return new Promise((resolve, reject) => {
      super.post({url:url+`?pageno=${pageno}&pagesize=${pagesize}`,  data: params})
        .then(response => {
          if (response.status === 200) {
            return resolve(response.data);
          } else {
   ...
   findList(params) {
    return new Promise((resolve, reject) => {
      super.post({url:this.url+"/findList", params})
        .then(response => {
          if (response.status === 200) {
        return    resolve(response.data);
          } else {
