...
const DzwjzxTransferApply = observer((props) => {
  ...
  const doSearchAction = (formData, tableStore) => {
    ...
    const params = { sw ,ztlx: props.lxzt};
  ...
  const doDownloadAction = (record) => {
    ...
    //新
    GszxyjcxStore.packEEP_2022(params).then(() => {
      const downloadparams = { packetfilepath: GszxyjcxStore.packetfilepath };
      debugger;
      GszxyjcxStore.downloadEep(downloadparams);
    });
  };
  ...
  const customTableAction = (text, record, index, store) => {
    return (
      <>
        {[
         record.wpid !== "ZZZZ" && ApplyStore.canWfDelete(record) && deleteTableAction(record, store),
    ...
    <>
      {sfjs && (
        <EpsRecordPanel
          ...
          initParams={{ sw: 'W', ztlx: props.lxzt,wfinst}}
        />
      )}
      {!sfjs && (
        <EpsRecordPanel
...
export default DzwjzxTransferApply;
