...
const ArchDetail = observer((props) => {
  ...
  const archstore = useLocalObservable(() => ({
    ...
    async initKtable() {
      ...
      //   }
      console.log('childktable111111111',childKtable);
      ...
      runInAction(() => {
        ...
        archstore.childColumns = childKfields
        ...
        console.log( 'childktable22',childKtable);
        if (props.setDakid) {
  ...
  useEffect(() => {
    ...
    console.log('111', archstore.childParams);
    if (storeTable && archstore.childParams.bmc && storeTable.findByKey) {
      storeTable.findByKey('', 1, storeTable.size, {
        ...store.childParams,
        fid: props.recordId,
      });
    }
  }, [archstore.childParams]);

  useEffect(() => {
    // if (archParams.dakid) {
      archstore.initKtable(props.recordId);
    //   }
  }, [props.recordId]);
  ...
  const customTableAction = (text, record, index, store) => {
    ...
    return [
      ...
      <EpsFilesView
        fjs={record.fjs}
        bmc={archstore.childKtable?.bmc}
        tmid={record.id}
        printfile={0}
        dakid={archstore.childKtable?.id}
