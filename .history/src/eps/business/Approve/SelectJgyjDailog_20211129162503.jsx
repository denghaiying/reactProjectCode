...
const SelectJgyjDailog = observer(props => {
  ...
  const { params, callback, closeModal, umid = 'DAGSYJ0009', extendParams = {} } = props;
  const { spcode = "gsyjsqd", spName = "档案进馆移交", spUrl,ids="" } = extendParams
  ...
  useEffect(() => {
    ...
    SelectDialStore.setUrl(`/eps/control/main/${spUrl}`)
    debugger
    SelectDialStore.queryForList({ yjzt: 'I', yjrmc: SysStore.getCurrentUser().yhmc, dakid: params.dakid });
    SelectDialStore.setSaveParams("title",`档案进馆移交`+ `-${params.mc}-${curdate.format("YYYY")}${curdate.format("MM")}${curdate.format("DD")}`);
