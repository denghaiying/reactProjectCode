...
import { Select, DatePicker, Space, Row, Col, Button, notification, Form, message,Tooltip, Tabs ,Modal, Progress,Icon , Input } from 'antd';
...
const TransferApply = observer((props) => {
  ...
  const ApplyService = new _ApplyService(`/api/eps/control/main/${props.spurl}`);
  const intl = useIntl();
  ...
  const defaultDetailColumns: any[] = [
    {
      ...
      title: "审批意见",
      width: "150px",
      render: (value, record, index) => <Space size={8}>
        <span>同意<span style={{ color: "#f50" }}>({record.kfs})</span></span>
        <span>不同意<span style={{ color: "#2db7f5" }}>({record.bkfs})</span></span>
      </Space>
  ...
  const [wflwType, setWflwType] = useState(["submit", "return", "reject", "logview"]);
  ...
  const showJstion = async () => {
    ...
    if (id === "1") {
      message.warning({ type: 'warning', content: '请选择条目信息' })
       ...
       const reponse = await fetch.post(`/api/eps/control/main/gsyjjssqd/updateJssj`, fd);
  ...
  */
  const editTableAction = (record, store) => {
    return (
      <EpsModalButton key={`approveEdit_${record.id}`} isIcon={true} store={store} params={{ jgmxid: record.id }} url='/perfortest/jcjg/sxjcjgLog' title="编辑" width={1200} height={400} name="详情"
        icon={<EditOutlined />}></EpsModalButton>
    )
  }
  ...
  const customActionjs = (store, ids) => {
    ...
    <Form
      ...
      >
        <Form.Item name="sw">
          <Select style={{ width: 120 }}>
           <Select.Option value="J">待接收</Select.Option>
            <Select.Option value="W">待我处理</Select.Option>
            <Select.Option value="H">我已处理</Select.Option>
          </Select>
        </Form.Item>
  ...
  const customTableAction = (text, record, index, store) => {
    return (<>
      ...
      {yjjs && record.wpid==="ZZZZ" &&
       <Button size="small" style={{fontSize: '12px'}} type="primary" shape="circle" icon={<DownloadOutlined />}
      ...
      <div className="detail-container">
        ...
        >
          <TabPane key={1} tab={"条目明细"} style={{ height: '100%' }} closable={false} >
            <ApplyDetail setTableStore={(value) => { setDetailTableStore(value) }} jdlx={props.wfCode} approve={ApplyStore.spkj} columns={applyDetailColumns} fid={ApplyStore.fid} detailParams={ApplyStore.detailParams} spurl={props.spurl} />
          </TabPane>
        ...
        <div className="banner">
            <div className="inner">
                <div className="bottom">
                    ...
                    <div className="steps">
                        <li className={ZxyjStore.jsprogressValue > 10 ? "first common on" : "first common"}>
                        ...
                        <li className={ZxyjStore.jsprogressValue > 30 ? "second common on" : "second common"}>
                            <div className="node-d">
                                <span className="node"><span className="el-icon-check"></span></span>
                                <img src="/api/eps/xsxjc/images/column-line.png"></img>
                        ...
                        <li className={ZxyjStore.jsprogressValue > 80 ? "third common on" : "third common"}>
                            <div className="node-d">
                                <span className="node"><span className="el-icon-check"></span></span>
                                <img src="/api/eps/xsxjc/images/column-line.png"></img>
                        ...
                        <li className={ZxyjStore.jsprogressValue === 100 ? "fourth common on" : "fourth common"}>
                            <div className="node-d">
                                <span className="node"><span className="el-icon-check"></span></span>
                                <img src="/api/eps/xsxjc/images/column-line.png"></img>
