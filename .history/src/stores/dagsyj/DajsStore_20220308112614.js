...
class DajsStore extends BaseStore {
  ...
  @action xduoJc = async (paramid) => {
    ...
    fetch.post(`/api/eps/control/main/gsyjsqd/duoJc`, fd, { responseType: 'json' })
      .then((resp) => {
        if (resp.status === 200) {
          ...
          if (timer == null) {
            timer = setInterval(() => {
              ...
              fetch.get(`/api/eps/control/main/gsyjsqd/queryForId`, { params })
                .then((gszxyjsqd) => {
  ...
  @action xdownloadEEP = async (params) => {
    ...
    await fetch
      ...
      .then((res) => {
        ...
        console.log(err)
        this.downloadLoading = false;
        message.error('下载失败！'+err.response.data);
      });
  };
}
