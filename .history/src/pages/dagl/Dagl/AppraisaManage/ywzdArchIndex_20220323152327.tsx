...
const YwzdArchManage = observer((props) => {
  ...
  const archStore: ArchStoreType = useLocalObservable(() => ({
    ...
    async initArchInfo(archParams: ArchParams, ktable) {
      ...
      let dakopt =
        dakOptAll &&
        dakOptAll.filter((item) => item.isadd === 'N' && item.optlx === '1');
      dakopt = dakopt?.filter((item) => item.fz !== 'DAKFZ001');

      let dakyhOpt =
        dakOptAll &&
        dakOptAll.filter((item) => item.isadd === 'Y' && item.oplx === '1');

      dakyhOpt = dakyhOpt && dakyhOpt.filter((item) => item.fz !== 'DAKFZ001');
      if (archParams.wzlk == 'Y') {
        dakopt['wzlk'] = 'Y';
        dakyhOpt['wzlk'] = 'Y';
      }
      ...
      runInAction(() => {
        ...
        archParams.psql = getParamSql(psql);
        this.ktable = ktable;
    ...
    // //文件移交 (之前套接新界面,暂未实现)
    // DAK0131: (optcode: string, params: ArchParams, archStore: ArchStoreType, ids: Array) => {
    //   var idss = [];
    //   var sr = archStore.selectRecords;
    //   for (var i = 0; i < sr.length; i++) {
    //     let insp = sr[i].insp;
    ...
    //   return { url: `/api/eps/control/main/dagl/Yjsp/SelectDailog`, params: filter, title: "移交文件", filter, width: 650, height: 550 };
  ...
  var newcqbcDataRecovery = async (param) => {
  ...
  var getParamValue = function (key) {
    var json = { table: archStore.ktable?.bmc };
  ...
  var getParamSql = function (asql) {
    ...
    if (r) {
      for (var i = 0; i < r.length; i++) {
        ...
        debugger;
        sql = sql.replace(reg, getParamValue(key.substring(1, key.length - 1)));
      }
    }
    return sql;
  };
  ...
  const refreshPage = async () => {
    ...
    await storeTable.findByKey('', 1, storeTable.size, {
      fid: props.fid,
      ...props.archParams,
    });
  };
