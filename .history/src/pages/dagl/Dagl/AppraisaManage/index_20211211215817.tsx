  ...
  const archStore: ArchStoreType = useLocalObservable(() => ({
    ...
    async initArchInfo(archParams: ArchParams, ktable) {
      ...
      let dakopt =
        ...
        dakOptAll.filter(
          (item) =>
            item.isadd == 'N' && item.optlx == '1' && item.fz != 'DAKFZ001',
        );
      ...
      let dakyhOpt =
        ...
        dakOptAll.filter(
          (item) =>
            item.isadd == 'Y' && item.oplx == '1' && item.fz != 'DAKFZ001',
        );
      // 用户自定义按钮
    ...
    async DAK0133(
      ...
      if (eepnres.value == 'Y') {
        window.open(
          '/api/eps/control/main/eepgl/downloadPakage?dakid=' +
           params.dakid + '&yhmc= ' +dakoptService.yhmc+'&dwmc='+SysStore.getCurrentCmp().dwname+
           '&ids=' +ids+'&mbid=' +archStore.ktable?.mbid+'&xxtmc=' +util.getLStorage('sysname')
        );
      } else {
        var filter = {
          yhid: dakoptService.yhid,
