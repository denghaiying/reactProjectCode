...
import { Form, Input, message, Select, Checkbox, Row, Col, Button, Modal, Divider, Radio, InputNumber } from 'antd';
...
const lxTypeData =[{value: '0', label: '长期保存'},{value: '1', label: '其他'}];
...
const customForm = () => {
  ...
  return (
    <>
      ...
      <Form.Item label="总容量:" name="zrl" required rules={[{ required: true, message: '请输入总容量' }]}>
        <InputNumber name="zrl"  type="inline" stringMode step={0.5}  min={0}  max={2000}   style={{ width:  _width-10 }} addonAfter="G" />
      </Form.Item>
      <Form.Item label="类型:" name="lx" required rules={[{ required: true, message: '请选择类型' }]}>
          <Select   placeholder="文件存储"  options={lxTypeData} style={{width:  _width}}/>
      </Form.Item>
      <Form.Item label="状态:" name="zt" required rules={[{ required: true, message: '请选择状态' }]}>
          <Select allowClear placeholder="状态" options={uingTypeData} style={{width:  _width}} />
      </Form.Item>
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return ([
      <>
        <CqbcZtcl record={ids} store={store} />
      </>
    ])
  }
