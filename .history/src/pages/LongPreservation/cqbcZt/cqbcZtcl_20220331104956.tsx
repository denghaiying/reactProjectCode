...
const cqbcZtcl = observer((props) => {
    ...
    const updateFunction = async (arr) => {
        ...
        ztclService.updateZtclsyxy(ztclStore.ztId,ztclStore.getID, arr, ztclStore.yhMc, ztclStore.yhId).then(res => {
            ...
            ztclService.queryztclList(ztclStore.ztId,ztclStore.getID).then(rigthres => {
                if (rigthres.status === 200) {
                    if (rigthres.data.length > 0) {
                        var fzrData = [];
                        for (var i = 0; i < rigthres.data.length; i++) {
                            let newKey = {};
                            newKey = rigthres.data[i];
