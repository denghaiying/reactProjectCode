...
import jkpzService from './service/CqbcJkpzService';
...
const [dzwjjkpzlist, setDzwjjkpzlist]= useState<Array<{id:string;label:string;value:string}>>([]);
...
const customForm = () => {
  ...
  return (
    <>
      ...
      <Form.Item label="电子文件中心接口:" name="dydzwjjkid" required rules={[{ required: true, message: '请选择电子文件中心接口' }]}>
         <Select   placeholder="电子文件中心接口"   options={dzwjjkpzlist} style={{width:  _width}}/>
      </Form.Item>
  ...
  useEffect(() => {
      ...
      const queryDzwjjkpzList =  async () =>{
        let url="/api/eps9/tyjk/jkpz/findForKey";
        const response =await fetch.post(url,{qyusing:'Y',type:2});
            if (response.status === 200) {
              if (response.data.length > 0) {
                let  mbData = response.data.map(o => ({ 'id': o.id, 'label':o.bh+"|"+o.name, 'value': o.id}));
                setDzwjjkpzlist(mbData);
              }
            }
      }
      queryCqbcDaKList();
