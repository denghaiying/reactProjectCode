...
const cqbcZjDak = observer((props) => {
  /**
 * 获取单位名称
 * @param {*} val
 */
  ...
  const localStore = useLocalObservable(() => (
    {
      ...
      async queryDakDw() {
        const response = await fetch.get(`/api/eps/control/main/dak/queryDwDak?ydwid=${this.ydw_id}&ydwmc=${this.ydw_mc}&dwbh=${this.dwbh}&dwmc=${this.dwmc}&pageIndex=${this.page_No - 1}&pageSize=${this.page_Size}&page=${this.page_No - 1}&limit=${this.page_Size}`);
        if (response.status === 200) {
          if (response.data.total > 0) {
            var sjData = [];
            for (var i = 0; i < response.data.total; i++) {
            ...
            this.DakDwData = sjData;
            this.DakDwTotal = response.data.total;
          }
          return;
        }
      },
  ...
  */
  ...
  const [addcqbc_visible, setAddCqbcVisible] = useState(false)

  /**
 * 调用后端复制方法
 */
  const onPut_Add = async () => {
    console.log("dwIds.length",dwIds.length)
    if (dwIds.length <= 0) {
  ...
  //点击事件
  const clickXDak = () => {
    localStore.queryDakDw();
    setAddCqbcVisible(true);
  }

  const CancelXDak = () => {
    debugger
    setAddCqbcVisible(false);
    localStore.queryTreeDwList();
  }
 ...
 * 调用后端生成方法
 */
    const onPut_AddCqbc = async () => {
      console.log("dwIds.length",dwIds.length)
      if (dwIds.length <= 0) {
        message.error("请至少选择一行数据!")
      } else {
        localStore.loading = true;
        const response = await fetch.get(`/api/eps/control/main/dak/copyDakscqbc?srcdwid=${localStore.ydw_id}&desdws=${dwIds}`);
        if (response.data.success) {
          message.success('操作成功');
          localStore.loading = false;
        } else {
  ...
  const customAction = () => {
    return (
      <>
        ...
        {/* <Button type="primary" icon={<CopyFilled />} onClick={() => clickXDak()}> 生成长期保存档案库 </Button>
        <Modal title={<span className="m-title">生成长期保存档案库</span>}
          visible={addcqbc_visible}
          onOk={onPut_AddCqbc}
          onCancel={CancelXDak}
          style={{ top: 50 }}
          width='1000px'
        >
          ...
          <div className="table">
            ...
            <Spin spinning={localStore.loading} tip="正在生成,请稍后......" size="large">
              <Table
                className="ant-table"
                rowSelection={{ type: selectionType, ...rowSelection, }}
                columns={columns}
                dataSource={localStore.DakDwData}
                bordered
                size="middle"
                //loading={localStore.loading}
                scroll={{ x: 900, y: 320 }}
                ...
                pagination={{
                  ...
                  defaultCurrent: 1,
                  defaultPageSize: 50,
                  showTotal: () => `共${localStore.DakDwTotal}条`,
                  pageSize: localStore.page_Size,
                  current: localStore.page_No,
                  total: localStore.DakDwTotal,
                  onChange: (current, pageSize = 50) => {
                    localStore.page_No = current;
                    localStore.page_Size = pageSize;
                    localStore.queryDakDw();
  ...
  return (
    <EpsPanel
      ...
      tableService={CqbcDakService}             // 右侧表格实现类，必填
      ref={ref}                                // 获取组件实例，选填
      formWidth={800}
