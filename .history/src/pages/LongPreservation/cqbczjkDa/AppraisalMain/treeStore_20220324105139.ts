...
class TreeStore extends BaseStore {
  ...
  queryKkjsPage = async () => {
    this.kksjloading = true;
      const response = await fetch.get(`/api/eps/control/main/kkjs/queryForPage?dakzt=${this.tmzt}&key=${this.key}&dw=${this.dw}&dakid=${this.dakid}&pageIndex=${this.page_No - 1}&pageSize=${this.page_Size}&page=${this.page_No - 1}&limit=${this.page_Size}`);
      if (response && response.status === 200) {
        debugger
        runInAction( () => {
  ...
  setExpand = (expend) => {
   ...
   findDakList = async () => {
    ...
    const res = await fetch
        .get(`/api/eps/control/main/dak/queryTree`, {params});
   ...
   queryTree = async () => {
    ...
    if (!this.oldver) {
      ...
      const response = await fetch.post(`${this.url}/queryTreeNew`, this.params, {
