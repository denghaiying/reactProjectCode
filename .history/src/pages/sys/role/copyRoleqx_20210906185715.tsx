    ...
    const copyRoleStore = useLocalObservable(() => (
        {
            ...
            async queryDwList() {
                ...
                if (this.roleCode === 'SYSROLE01' || this.roleCode === 'SYSROLE02') {
                    const response = await fetch.get(`/api/eps/control/main/dw/queryForList_e9_superUser_roleCopy?dwbh=${this.dwbh}&dwmc=${this.dwmc}&pageIndex=${this.page_No - 1}&pageSize=${this.page_Size}&page=${this.page_No - 1}&limit=${this.page_Size}`);
    ...
    const columns = [
        {
            ...
            align: 'center',
            width: 30,
            render: (_, __, index: number) => index + (RoleStore.page_No - 1) * RoleStore.page_Size + 1,
        },
        {
            title: '编码',
            dataIndex: 'dwbh',
            ...
            {() => (
                <>
                    <Tooltip title="复制到其他单位" key={"copyRole" + index}>
                        <Button size="small" style={{ fontSize: '12px' }} type="primary" shape="circle" icon={<CopyOutlined />} onClick={() => checkButton()} />

                        <Modal title={<span className="m-title">复制角色</span>}
                            visible={add_visible}
                            onOk={onPut_Add}
                        ...
                        >
                            <div >
                                <Input style={{ width: 180 }} allowClear name="dwbh" placeholder="请输入单位编号" onChange={getDwbh}></Input>
                                &nbsp; &nbsp;
                                <Input style={{ width: 180 }} allowClear name="dwmc" placeholder="请输入单位名称" onChange={getDwmc}></Input>
                                &nbsp; &nbsp; &nbsp; &nbsp;
