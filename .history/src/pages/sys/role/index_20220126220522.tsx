...
import Sjqx from '@/pages/base/sjqx';
...
const Role = observer((props) => {
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return ([
      <>
         <EpsModalButton name="业务权限" title="业务权限" width={1200}   useIframe={true}  url={'/api/eps/control/main/dakqx/ywqxszMainx'}   beforeOpen={(ids) => onywqxchange(ids)}   params={{code:ids[0]?.code,dwid:ids[0]?.dwid,edittype:ids[0]?.edittype,
  ...
  return (
    <>
      ...
      <Modal
        ...
        >
        <div style={{maxHeight:  (props.height || modalHeight) + 'px', height: ( props.height || modalHeight) + 'px', width: (width - 48) + 'px'}}>
        <Sjqx/>
        </div>
      </Modal>
