...
const Role = observer((props) => {
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return ([
      <>
         <EpsModalButton name="业务权限1" title="业务权限1" width={1200}   useIframe={true}  url={'/api/eps/control/main/dakqx/ywqxszMainx'}   beforeOpen={() => onywqxchange(ids)}   params={{ record:{code:ids[0]?.code,dwid:ids[0]?.dwid,edittype:ids[0]?.edittype,
