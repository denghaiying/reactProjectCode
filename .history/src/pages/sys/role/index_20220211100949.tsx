...
const Role = observer((props) => {
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return ([
      <>
         <EpsModalButton name="业务权限" title="业务权限" width={'100%'}
            height={document.body.clientHeight - 90}  modalStyles={{ top: 0 }}  useIframe={true}  url={'/api/eps/control/main/dakqx/ywqxszMainx'}
               beforeOpen={(val) => {
                if (val.length != 1) { message.error('操作失败,请选择一行数据'); return false;} else {childStore.roleid=val[0].id; return true;}
           }}
