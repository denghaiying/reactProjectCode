...
import { Form, Input, Select, TreeSelect, Button, message } from 'antd';
...
const Role = observer((props) => {
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return ([
      <>
         <EpsModalButton name="业务权限1" title="业务权限1" width={1200}   useIframe={true}  url={'/api/eps/control/main/dakqx/ywqxszMainx'}   beforeOpen={() => onywqxchange(ids)}   params={{ record:childStore.selectdata,dwid:RoleStore.dw_id}} icon={<SettingOutlined />}/>

        <Button type="primary" onClick={() => onButtonClick()}>业务权限</Button>
        <Button type="primary" onClick={() => onButtonClick()}>数据权限</Button>
      </>
    ])
  }
