...
const Login: React.FC = observer(() => {
  ...
  const setCurrentCmpInfo = async (dwid) => {
    const dw = await queryCmp(dwid)
    util.setLStorage("currentCmp", dw);
  }

  const fetchUserInfo = async () => {
    util.clearLStorage("currentUser");
    util.clearLStorage("currentCmp");
    const userInfo = await initialState?.fetchUserInfo?.();
    util.setLStorage("currentUser", userInfo);
    setCurrentCmpInfo(userInfo.dwid);
  ...
  const handleSubmit = async (values: API.LoginParams) => {
    ...
    setSubmitting(true);
    try {
      const token = await UserService.login(values.username, diagest.md5(values.password));
        ...
        <div className={styles.top}>
          <div className={styles.header}>
            <Link to="/">
              ...
              <span className={styles.title}>{LoginStore.xtname}</span>
            </Link>
          </div>
