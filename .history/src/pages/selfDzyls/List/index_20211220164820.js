...
const List = observer((props) => {
  const store = useLocalStore(() => ({
    ...
    async findaddress() {
      const response = await fetch.post(`/api/streamingapi/content/findForPage?pageno=1&pagesize=20&contentchannelid=`);
  ...
  return (
    <div className={`${style['list-content']}`}>
        ...
        <div className={`${style['list-body']}`}>
          <div className={`${style['left-menu']}`}>{this.renderMenu()}</div>
