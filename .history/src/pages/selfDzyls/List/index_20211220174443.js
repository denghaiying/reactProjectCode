...
const List = observer((props) => {
  const store = useLocalStore(() => ({
    ...
    async findaddress() {
      const res = await ListService.findByKey("",1,50,{lxtype:'dwjs'});
      debugger
      this.lists=res.list;
      // axios.get(`/api/streamingapi/content/findForPage?pageno=1&pagesize=20`, {params: {city:'dwjs'}})
      // .then(result => {
      //   debugger
      // })
    },
        ...
        <div className={`${style['list-body']}`}>
          ...
          <div className={`${style['list-table']}`}>
            ...
            <div className={`${style['pagination']}`}>
              <Pagination defaultCurrent={1} total={store.lists.length()} />
              <span className={`${style['total']}`}>共{store.lists.length()}页</span>
            </div>
          </div>
        </div>
        <Technology />
