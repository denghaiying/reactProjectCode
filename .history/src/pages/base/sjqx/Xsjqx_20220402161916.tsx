...
import autoHeight from '../../dashboard/monitor/components/Charts/Gauge/index';
    ...
    const sjqxStore = useLocalObservable(() => (
        {
            ...
            async queryQwtmgzListAll() {
                const response = await fetch.get(`/api/eps/control/main/sjqxgz/queryForList`);
                if (response && response.status === 200) {
                    this.qwtmgzlstALL = response.data.map(o => ({ 'id': o.id, 'label': o.code+'|'+o.name, 'value': o.id }));
...
const customTableAction = (text, record, index, store) => {
   ...
   */
    ...
    useEffect(() => {
        ...
        sjqxStore.queryQwtmgzListAll();
        sjqxStore.getMkReg();
        sjqxStore.getRolerenderlist(props.params.dwid);
        sjqxStore.queryQjjsList(props.params.dwid);
        
        setroleid(props.params.roleid);
    }, []);
        ...
        return (
            <>
                <Row gutter={20}>
                    ...
                    <Col span={span}>
                        <Form.Item label="角色:" name="roleid" initialValue={xroleid}>
                            <Input disabled  className="ant-input"  hidden style={{width:  _width}} />
                        </Form.Item>
                    </Col>
                    <Col span={span}>
    ...
    const source: EpsSource[] = [
        ...
        },
        {
            ...
            render: (text, record, index) => {
                    ...
                    let aa = sjqxStore.qwtmgzlstALL.filter(item => {
                        return item.value === text;
                    })
                    return aa[0]?.label;
