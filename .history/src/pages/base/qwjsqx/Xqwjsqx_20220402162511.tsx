     ...
     */
     const qwjsqxStore = useLocalObservable(() => ({
        ...
        async queryQwtmgzListAll() {
          ...
          if (response && response.status === 200) {
            this.qwtmgzlstAll = response.data.map((o) => ({
              id: o.id,
              label: o.code + '|' + o.name,
              value: o.id,
            }));
...
const customTableAction = (text, record, index, store) => {
   ...
   */
    ...
    useEffect(() => {
      debugger
        ...
        qwjsqxStore.queryQjjsList(props.params.dwid);
        qwjsqxStore.queryQwtmgzListAll();
      ...
      <>
        <Row gutter={20}>
          ...
          <Col span={span}>
            <Form.Item
              label="角色:"
              name="roleid" initialValue={xroleid} 
            >
  ...
  const source: EpsSource[] = [
    ...
    },
    {
      ...
      formType: EpsFormType.Input,
      render: (text, record, index) => {
          let aa = qwjsqxStore.qwtmgzlstAll.filter((item) => {
            return item.value === text;
          });
          return aa[0]?.label;
      },
