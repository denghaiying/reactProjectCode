  ...
  const dakStore = useLocalObservable(() => (
    {
        ...
        async queryCqbcJkList() {
            const response = await fetch.post(`/api/eps/lg/cqbcjkpz/findForKey`,{qyusing:'Y'});
            if (response.status === 200) {
                if (response.data.length > 0) {
                    let  SxData = response.data.map(o => ({ 'id': o.id, 'label':o.bh+"|"+o.name, 'value': o.id }));
                    this.alljkMata=SxData;
                }
                return;
            }
        },
        ...
        <>
            ...
            <Modal title={<span className="m-title">修改档案库</span>}
                ...
                <Form labelCol={{ span: 8 }} form={form} className="schedule-form" name="shForm" >
                    ...
                    <FormItem label="长期保存接口:" name="cqbcjkid" initialValue={dakStore.DakData.cqbcjkid}>
                    <Select
                            style={{ width: 250 }}
                            placeholder="长期保存接口"
                            options={dakStore.alljkMata}
                        />
                    </FormItem>
