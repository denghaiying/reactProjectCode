    ...
    const dakStore = useLocalObservable(() => (
        {
            ...
            async queryDzwjJkList() {
                const response = await fetch.post(`/api/eps/lg/zjkjkpz/findForKey`,{qyusing:'Y',type:2});
                if (response.status === 200) {
                    if (response.data.length > 0) {
            ...
            <Modal title={<span className="m-title">修改档案库</span>}
                ...
                <Form labelCol={{ span: 8 }} form={form} className="schedule-form" name="shForm" >
                    <Row gutter={20}>
                      ...
                      <Col span={span}>
                        <FormItem label="启用催收与统计:" name="sjcstx" initialValue={dakStore.DakData.sjcstx === 'Y'}>
                          <Switch checkedChildren="开启" unCheckedChildren="关闭" defaultChecked={dakStore.DakData.sjcstx === 'Y'}/>
                        </FormItem>
                      </Col>
                      <Col span={span}>
                        <FormItem
                        ...
                        >
                            <Switch checkedChildren="开启" unCheckedChildren="关闭" defaultChecked={dakStore.DakData.cqbclh === 'Y'} />
                        </FormItem>
                        </Col>
                    </Row>
                </Form>
