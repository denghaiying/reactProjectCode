...
import { Button, Modal, message, Input, Table, Spin, Tag } from 'antd';
 ...
 */
  ...
  const findByDataLx = (record, index, store) => {
    ...
    if (record.type === 'D') {
      ...
      res.push(<Xy record={record} key={'xy' + index} store={store} />,)
      // if (record.lx == 'F') {
      //   res.push(<img src={tree1} />)
      // } else if (record.lx  == '01') {
      //   res.push(<img src={tree3} />)
      // }else if (record.lx  == '02') {
      //   res.push( <img src={tree2} />)
      // } else {
      //   res.push( <img src={tree3} />)
      // }
    }
  ...
  const getTreeNodeData = (node) => {
    ...
    node.label=node.icon+node.label
    debugger
  ...
  const source: EpsSource[] = [
    {
      ...
      render: (text, record, index) => {
        if (record.lx == 'F') {
          return <Tag icon={<img src={tree1}/>}>{text}</Tag>;
        }else if (record.lx == '01') {
          return <img src={tree3} alt={text}/><div>{text}</div>;
        }else if (record.lx == '02') {
          return <img src={tree2} alt={text}/>;
        } else {
          return <img src={tree3} alt={text}/>;
        }
      },
    }
  ]
