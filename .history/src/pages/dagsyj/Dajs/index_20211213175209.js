...
const Dajs = observer((props) => {
  ...
  const sjdata = async (val) => {
    ...
    const ressj =  await DajsService.sjdatajs({id:val,jsr:SysStore.getCurrentUser().yhmc,jsrid:SysStore.getCurrentUser().id,jsdwmc:SysStore.getCurrentCmp().mc,jsdw:SysStore.getCurrentCmp().id});
    debugger
    message.success('成功');
  };
  ...
  const sjJsction = async (val) => {
    ...
    //  const ressj =  await DajsService.sjdatajs({id:val});
      Modal.confirm({
        type: 'warning',
        title: `确定接收吗`,
        cancelText: '取消',
      ...
      message.warning({ type: 'warning', content: '不能接收，需先进行四性检测！' })
    }
  };
        ...
        <div className={`${style['table-area']} ${style['common-wrap']}`}>
          ...
          <div className={`${style['common-content']}`}>
            <div className={`${style['btns-area']}`}>
              <Button type="primary" className={`${style['btns']}`} onClick={() => doztAction("D")} >
                待接收
              </Button>
              <Button type="primary" className={`${style['btns']}`} onClick={() => doztAction("Y")} >
                已接收
              </Button>
              <Button type="primary" className={`${style['btns-type']}`} onClick={() => doztAction("Q")} >
                全部
              </Button>
            </div>
            <Table
        ...
        <div className="banner">
            <div className="inner">
                <div className="bottom">
                    ...
                    <div className="steps">
                        <li className={DajsStore.progressValue > 10 ? "first common on" : "first common"}>
                        ...
                        <li className={DajsStore.progressValue > 30 ? "second common on" : "second common"}>
                            <div className="node-d">
                                <span className="node"><span className="el-icon-check"></span></span>
                                <img src="/api/eps/xsxjc/images/column-line.png"></img>
                        ...
                        <li className={DajsStore.progressValue > 80 ? "third common on" : "third common"}>
                            <div className="node-d">
                                <span className="node"><span className="el-icon-check"></span></span>
                                <img src="/api/eps/xsxjc/images/column-line.png"></img>
                        ...
                        <li className={DajsStore.progressValue === 100 ? "fourth common on" : "fourth common"}>
                            <div className="node-d">
                                <span className="node"><span className="el-icon-check"></span></span>
                                <img src="/api/eps/xsxjc/images/column-line.png"></img>
        ...
        <div className="list-content">
            <div className="correct">
                ...
                <div className="icon-style-demo">
                    ...
                    <code style={{color: "#FF3333", marginRight: "10px"}}>{DajsStore.jcjg}</code>
                </div>
                }
            </div>
        </div>
