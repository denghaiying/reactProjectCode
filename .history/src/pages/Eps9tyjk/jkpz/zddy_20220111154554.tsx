...
import { Form, Input, message, Select, Checkbox, Modal, Table} from 'antd';
  ...
  return (
    <>
      ...
      <Form.Item label="档案库字段名:" name="dakzd">
         <Select   placeholder="档案库字段名"   options={daklist} style={{width:  _width}}/>
      </Form.Item>
      <Form.Item label="中间库字段:" name="zjkzd">
        <Select   placeholder="中间库字段"   options={zjklist} style={{width:  _width}}/>
      </Form.Item>
      ...
      <Form.Item label="默认值:" name="zdmrz">
       ...
       <Form.Item label="字段类型:" name="zdlx" required rules={[{ required: true, message: '请选择字段类型' }]}>
        <Select   placeholder="字段类型"   options={lxData}  style={{width:  _width}}/>
      </Form.Item>
      ...
      <Modal
        ...
        >
          ...
          <div style={{ height: '100%'}}>
            <Table  className="ant-table" rowSelection={rowMxSelection} columns={columns} dataSource={zlxlist} size="middle"/>
          </div>
               {/* onRow={record => {
                    return {
                      onClick: event => {
