...
const [viswitch, setSwitch] =useState(false);
...
const customForm = () => {
  ...
  return (
    <>
      ...
      <Form.Item label="档案库类型:" name="sjkdb" required rules={[{ required: true, message: '请选择档案库类型' }]}>
         <Select   placeholder="档案库类型"   options={dbData} style={{width:  _width}}/>
      </Form.Item>

      <Form.Item label="ftp路径:" name="ftppath" required rules={[{ required: true, message: '请输入ftp路径' }]}>
        <Input allowClear style={{ width: _width }} />
      </Form.Item>
        <Form.Item label="启用:" name="qyusing">
           <Switch checkedChildren="开启" unCheckedChildren="关闭" checked={UserStore.editRecord.uenable}/>
      </Form.Item>
      <Form.Item label="类型:" name="lxtype"  required rules={[{ required: true, message: '请选择类型' }]}>
