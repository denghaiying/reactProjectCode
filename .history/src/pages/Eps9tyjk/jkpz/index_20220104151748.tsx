...
const lxData = [{value: "1", label: "数据同步"},{value: "2", label: "用户同步"}, {value: "3", label: "组织机构同步"}, {value: "4", label: "数据导入到表"}, {value: "5", label: "AD域数据同步"},{value: "6", label: "温湿度远程数据同步"} ,{value: "7", label: "档案库到编研库同步"},{value: "8", label: "ASIP包更新"},{value: "9", label: "EEP包更新"}];
const dbData =[{value: "ORACLE", label: "ORACLE"},{value: "SQLSERVER", label: "SQLSERVER"}, {value: "MYSQL", label: "MYSQL"},{value: "H2", label: "H2"},{value: "KINGBASE8", label: "人大金仓"},{value: "DM", label: "达梦"}];
const cslx = [{value : "1", label : "ftp传输"}, {value : "2", label : "存储二进制传输"},{value: "3", label: "AWS3模式"}];
const sfyw = [{value : "是", label  : "是"},{value : "否", label  : "否"}];
const jklxData = [{value: "1", label: "ASIP包"},{value: "2", label: "中间库数据"},{value: "3", label: "EEP包"},{value: "4", label: "EEP中间库数据"}];
...
const customForm = () => {
  ...
  return (
    ...
    <Row gutter={20}>
      <Col span={span}>
        <Form.Item label="接口类型:" name="jklx" required  rules={[{ required: true, message: '请选择接口类型' }]}>
            <Select  allowClear  placeholder="接口类型"   options={jklxData} style={{width:  _width}}/>
        </Form.Item>
      </Col>
      <Col span={span}>
        <Form.Item label="中间库"  name="zjkid" required  rules={[{ required: true, message: '请选择中间库' }]}>
               <Select  allowClear  placeholder="中间库"   options={zjklist} style={{width:  _width}}/>
            </Form.Item>
      </Col>
      <Col span={span}>
      ...
      <Col span={span}>
        <Form.Item label="模版"  name="mb" required  rules={[{ required: true, message: '请选择模版' }]}>
               <Select  allowClear  placeholder="模版"   options={mblist} style={{width:  _width}}/>
            </Form.Item>
      </Col>
      <Col span={span}>
        <Form.Item label="附件传输类型"  name="lx" required  rules={[{ required: true, message: '请选择附件传输类型' }]}>
               <Select  allowClear  placeholder="附件传输类型"   options={cslx} style={{width:  _width}}/>
            </Form.Item>
      </Col>
      <Col span={span}>
        <Form.Item label="中间FTP地址:" name="ftpip" required  rules={[{ required: true, message: '请输入中间FTP地址' }]}>
          <Input allowClear style={{ width:  _width }} />
        </Form.Item>
      </Col>
      <Col span={span}>
        <Form.Item label="中间FTP端口:" name="ftpport" required  rules={[{ required: true, message: '请输入中间FTP端口' }]}>
          <Input allowClear style={{ width:  _width }} />
        </Form.Item>
      </Col>
      <Col span={span}>
        <Form.Item label="中间FTP用户名称:" name="ftpuname" required  rules={[{ required: true, message: '请输入中间FTP用户名称' }]} >
          <Input allowClear style={{ width: _width }} />
        </Form.Item>
      </Col>
      <Col span={span}>
        <Form.Item label="中间FTP密码:" name="ftppass"  required  rules={[{ required: true, message: '请输入中间FTP密码' }]}>
          <Input allowClear style={{ width: _width }} />
        </Form.Item>
      </Col>
      <Col span={span}>
      ...
      <Divider orientation="left" plain> 原文信息</Divider>
      <Col span={span}>
      ...
      <Divider orientation="left" plain> EEP信息</Divider>
      <Col span={span}>
      ...
      <Col span={span}>
        <Form.Item label="EEP数据接口" name="asipjkpzid">
           <Select   placeholder="ASIP数据接口"   options={asipjkpzlist} style={{width:  _width}}/>
        </Form.Item>
      </Col>
    <Divider orientation="left" plain> 其它信息</Divider>
      ...
      <Col span={span}>
        <Form.Item label="启用四性检测:" valuePropName="checked" name="sqjcqy">
            <Checkbox  style={{width:  _width}}/>
        </Form.Item>
      </Col>
      <Col span={span}>
