...
const customForm = () => {
  ...
  return (
    <>
      ...
      <Form.Item label="文件存储:" name="ftppath" required rules={[{ required: true, message: '请选择文件存储' }]}>
      ...
      <Form.Item label="接口类型:" name="type"  required rules={[{ required: true, message: '接口类型' }]} initialValue="1">
         <Radio.Group name="type">
            <Radio value="1">普通</Radio>
            <Radio value="2">长期保存</Radio>
          </Radio.Group>
      </Form.Item>
      <Form.Item label="解析器Bean名称:" name="xjbean">
        <Input allowClear style={{ width: _width }} />
      </Form.Item>
      <Form.Item name="qysxjc" label="四性检测:" initialValue="true">
          <Radio.Group>
            <Radio.Button value="true">开启</Radio.Button>
            <Radio.Button value="false">关闭</Radio.Button>
          </Radio.Group>
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return ([
      <>
        ...
        <Button type="primary" onClick={() => doCreateTemplateString(ids)}>生成样例</Button>
      </>
    ])
  }
  ...
  const searchFrom = () => {
    return (
      <>
        ...
        <Form.Item label="接口类型" className="form-item" name="lxtype">
          <Select allowClear placeholder="接口类型" options={lxTypeData} />
        </Form.Item>
