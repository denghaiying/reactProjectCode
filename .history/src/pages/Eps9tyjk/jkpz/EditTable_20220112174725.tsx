...
const EditStore = useLocalObservable(() => (
  {
      ...
      rowSelection() {
        let opts = {
            selectedRowKeys: this.rowSelected,
            onChange: (selectedRowKeys, selectedRows) => {
                // selectedRowKeys = rowKeys;
                console.log(selectedRowKeys);
                console.log(selectedRows);
                // console.log('选中对象:' + JSON.stringify(selectedRowKeys));
            },
        };
        return opts;
    }
...
// const rowSelection = {
//   selectedRowKeys: EditStore.dataSelectedRowskeys,
...
const changezd =  async (val) =>{
  ...
  if(val.dakzd==undefined){
      for(var i=EditStore.rowsData.length-1;i>-1;i--){
        if(EditStore.rowsData[i].id == val.id){
            EditStore.rowsData.splice(i,1);
        }
      }
      for(var i=EditStore.keysData.length-1;i>-1;i--){
        if(EditStore.keysData[i] == val.id){
            EditStore.keysData.splice(i,1);
        }
      }
  }else{
    EditStore.rowsData.push(val);
    EditStore.keysData.push(val.id);
  }
  EditStore.dataSelectedRowskeys=EditStore.keysData;
 // setdataSelectedRows(EditStore.rowsData);
}
  ...
  return (
    <>
     <EditableProTable<DataSourceType>
        ...
        rowSelection={EditStore.rowSelection()}
        toolBarRender={() => {
          return [
