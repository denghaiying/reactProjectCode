import { Input, Table, Tree, Tabs, message, Button  } from "antd";
import { observer } from "mobx-react";
import React, { useState, useEffect, useImperativeHandle, forwardRef } from "react";
import { EpsMainTableStore, EpsChildTableStore } from ".";
import EpsAddButton from "../../buttons/EpsAddButton";
import EpsDeleteButton from "../../buttons/EpsDeleteButton";
import EpsEditButton from "../../buttons/EpsEditButton";
import EpsTreeStore from "./EpsTreeStore";
// import AdvancedSearch from './advancedSearch'

import './index.less'
import { CarryOutOutlined, LeftOutlined, RightOutlined, UnorderedListOutlined } from "@ant-design/icons";
import { Icon, Tab } from "@alifd/next";
import { EpsDakProps, EpsSource, TabData, TableColumn } from "@/eps/commons/declare";

import Empty from '../../../../styles/assets/img/icon_empty.png'


const { Search } = Input;
const { TabPane } = Tabs;
const PerImgWidth = 42;         // 表格按钮列宽

const EpsPanel = observer(forwardRef((props: EpsDakProps, ref) => {

    const [treeSelectData, setTreeSelectData] = useState('')
    const [selectedRowKeys, setSelectedRowKeys] = useState([])
    const [clickRowData, setClickRowData] = useState();

    const onTreeSelect = props.onTreeSelect

    const treeCheckAble = props.treeProp?.treeCheckAble || false;


    // 左上 tab
    const tabData = props.tabProps?.tabData || []
    const [tabCheckKey, setTabCheckKey] = useState('')

    // 左侧树选择后，设置选中值
    const onSelect  = async e => {
      onTreeSelect ? onTreeSelect(e && e[0]) : setTreeSelectData(e && e[0]);
    }

    // 左侧树伸缩
    const [isExpand, setIsExpand] = useState(true);

    // 左侧表格搜索框
    const [treeSearch, setTreeSearch] = useState(true)
    // 右侧表格搜索框
    const [tableSearch, setTableSearch] = useState(true);
    const [drawVisible, setDrawVisible] =useState(false);
    // 创建左侧树Store实例
    const [treeStore] = useState<EpsTreeStore>(new EpsTreeStore(props.treeService))
    // 创建右侧表格Store实例
    const [tableStore] = useState<EpsMainTableStore>(new EpsMainTableStore(props.mainTableService))
    const [childTableStore] = useState<EpsChildTableStore>(new EpsChildTableStore(props.childTableService))
    const [columns, setColumns] = useState<TableColumn[]>([])
    const [childColumns, setChildColumns] = useState<TableColumn[]>([])
    const [treeHeight, setTreeHeight] = useState(0)

    // 暴露tableStore
    useImperativeHandle(ref, () => ({
      getTableStore: () => tableStore,
      getTreeStore: () => treeStore,
      clearTableRowClick: () => {setSelectedRowKeys([]); setCheckedRows([])}
    }));

    // 表格相关配置
    const enableEdit = !props.mainTableProp?.disableEdit;
    const enableDelete = !props.mainTableProp?.disableDelete;
    const enableAdd = !props.mainTableProp?.disableAdd;
    // 表格选中行时，设置key值
    const [rowSelection, setRowSelection] = useState(props.mainTableProp?.rowSelection);
    // 表格选中行时， 设置行属性
    const [checkedRows, setCheckedRows] = useState<Array<any>>([]);
    let actionWidth = 0 + (enableEdit ? PerImgWidth : 0) + (enableDelete ? PerImgWidth : 0);


    // 右侧菜单按钮控制
    const menuProp = props.mainMenuProp || undefined;
    if(rowSelection) {
      rowSelection['onChange'] =  (value, row ) => {setSelectedRowKeys(value); setCheckedRows(row)}
    }

    if (menuProp){
      // 使用menu，初始化 行选择数据
      if(!rowSelection) {
        setRowSelection({type: 'radio', onChange: (value, row) => {setSelectedRowKeys(value); setCheckedRows(row)} })
      }
    }

    // 自定义表单，用于替换默认表单
    const customForm = props.mainCustomForm || undefined;
    // 搜索表单，如有设置，则显示高级搜索
    const searchForm = props.mainSearchForm;
    // 自定义功能按钮
    const customAction = props.mainCustomAction || undefined;
    // 自定义表格行按钮
    const customTableAction = props.mainCustomTableAction || undefined;
    if(customTableAction){
        const {props : {children}}= customTableAction('',{},0, tableStore)
        if(children){
            if (Object.prototype.toString.call(children) === '[object Object]') {
                actionWidth += PerImgWidth
            }
            if(Array.isArray(children)) {
                actionWidth += children.length * PerImgWidth
            }
        }
    }
    useEffect(() => {
      setColumns(getTableColumn(props.source));
    }, [props.source])

    useEffect(() => {
      setChildColumns(getTableColumn(props.childSource));
    }, [props.childSource])

    // 获取表格列
    const getTableColumn = (list: Array<EpsSource>) => {
        const columns: Array<any> = [{
            title: '序号',
            align: 'center',
            fixed: 'left',
            width: 50,
            render: (_, __, index: number) => index + (tableStore.mainPage-1) * tableStore.mainSize +1,
        }];
        if(list){
            for (let item of list) {
               item && columns.push(new TableColumn(item.code, item.title, item.align, item.render, item.width, item.ellipsis, item.fixed));
            }
        }
        if (enableDelete || enableEdit || actionWidth > 0) {
            columns.push({
              title: '操作',
              align: 'center',
              key: 'action',
              fixed: actionWidth > 0 ? 'right' : false,
              width: actionWidth === PerImgWidth ? 50: actionWidth,
              render: (text,record,index) => (
                <>
                  {enableEdit && <EpsEditButton column={props.source} title={props.title.name} data={record} store={tableStore} customForm={customForm}/>}
                  {enableDelete && <EpsDeleteButton data={text} store={tableStore} />}
                  {customTableAction && customTableAction(text, record, index, tableStore)}
                </>
              ),
            });
          }
        return columns;
    }

    const showTab = (tabData: Array<TabData>) => {
      if (!tabData || tabData.length === 0){
        return (<></>)
      }
      return (
        <Tabs defaultActiveKey={tabCheckKey} onChange={key => setTabCheckKey(key)}>
          {tabData.map(item =>
            <TabPane tab={item.tab} key={item.key}></TabPane>
          )}
        </Tabs>
      )
    }

    let {treeList, loadAsyncData} = treeStore;
    treeList.map(item => {
      item.icon = <CarryOutOutlined />
      return item;
    })
    let {mainTableList, mainLoading, mainPage, mainSize, mainTotal} = tableStore
    let { childTableList, childLoading, childPage, childSize, childTotal } = childTableStore

    const onPageSizeChange = (page, size) => {
        page != tableStore.mainPage && (tableStore.mainPage = page);
        size !== tableStore.mainSize && (tableStore.mainSize = size);

    }

    // 左侧树区域展开收缩
    const [showMenus, setShowMenus] = useState(false);
    const [addBtnArr, setAddBtnArr] = useState<Array<any>>([])
    const [editFlag, setEditFlag]= useState(false)
    // const tempArr: Array<string> = []
    // for(let i = 0; i < 48; i++ ) {
    //   tempArr.push(`批量导入${i+1}`)
    // }
    // const colorArr = ['#30B0D5', '#CFA32A', '#3F8EFF', '#1DA291', '#D1C61C', '#7A8EFC', '#3F8EFF', '#1DA291', '#30B0D5', '#7956EF', '#3F8EFF', '#8DDE04', '#3F8EFF',
    //                   '#1DA291', '#30B0D5', '#2C75DE', '#3F8EFF', '#1DA291', '#30B0D5', '#2C75DE', '#3CA2E3', '#1DA291', '#30B0D5', '#3F8EFF', '#2C75DE', '#94C343',
    //                   '#7956EF', '#3F8EFF', '#1DA291', '#94C343', '#FB9770', '#3F8EFF', '#8DDE04', '#1DA291', '#D1C61C', '#4BB4C3', '#1DA291', '#FB9770', '#259BAD',
    //                   '#1DA291', '#94C343', '#4B98E8', '#28A0B7', '#3F8EFF', '#30B0D5', '#3F8EFF', '#30B0D5', '#1DA291', '#7A8EFC', '#2C75DE', '#94C343', '#1DA291']


    const clickAdd = (item) => {      // 添加菜单
      let newAddArr: Array<any> = [...addBtnArr]
      if(newAddArr.includes(item)) {
        message.warning({type: 'warning', content: '请勿重复添加'})
        return
      } else {
        newAddArr.push(item)
        setAddBtnArr(newAddArr)
      }
    }

    const delBtn = (item) => {      // 删除菜单
      let newAddArr = [...addBtnArr]
      newAddArr.splice(newAddArr.findIndex(val => val === item), 1)
      setAddBtnArr(newAddArr)
    }


    useEffect(() => {
        tableStore.findByKey(treeSelectData, tableStore.mainPage, tableStore.mainSize, tableStore.mainParams);
    }, [mainPage, mainSize])

    useEffect(() => {
      if(clickRowData){
        childTableStore.findByKey(clickRowData.id)
      }
    }, [clickRowData])

    useEffect(() => {
      props.tabProps?.onTabChange(tabCheckKey);
    }, [tabCheckKey])

     //初始化按钮栏目按钮
     useEffect(() => {
      let newAddArr = [...menuProp]
      setAddBtnArr(newAddArr.filter(item=>item.toolbarShow))
    }, [menuProp])

    const onTableSearch = (tableSearchValue) => {
      tableStore.findByKey(tableStore.mainKey, 1, tableStore.mainSize, {'key': tableSearchValue});
    }

    let _height = window.innerHeight - 316

    useEffect(() => {

      // 设置左侧tab
      if (tabData && tabData.length >0){
        setTabCheckKey(tabData[0].key)
      }

      let _flag = props.treeProp?.treeSearch === undefined ? true : props.treeProp?.treeSearch
      if(_flag){
          _height = _height - 20
      }
      // 设置左侧树高度
      setTreeHeight(_height)
      // 是否显示左侧树查询框
      setTreeSearch(props.treeProp?.treeSearch === undefined ? true : props.treeProp?.treeSearch)         //左侧树搜索框
      // 是否显示右侧表格查询框
      setTableSearch(props.mainTableProp?.tableSearch === undefined ? true : props.mainTableProp?.tableSearch)       //右侧树搜索框
      // 设置表格列

      treeStore.findTree({});
      treeStore.treeList = treeStore.treeList
    }, [])

    // 选中值变化时，重新渲染右侧表格
    useEffect(() => {
        if(mainPage != 1){
            tableStore.mainPage = 1
        }
        tableStore.findByKey(treeSelectData, tableStore.mainPage, tableStore.mainSize, tableStore.mainParams)
    }, [treeSelectData])

    return(
        <>
        <div className="dak-manage">
          {
            showTab(tabData)
          }

          <div className={isExpand ? 'isExpand main-content' : 'main-content'}>
            <div className='left-tree'>
              <div className="collapse-tree">
              {
                  // 左侧树搜索框
                  treeSearch &&  <Search placeholder="搜索" type="normal"  style={{ width: '100%' }} onSearch={key => treeStore.findTree(key)}/>
              }
              {
                !treeList.length ? <div className="empty"><img src={Empty}/><p>暂无数据</p></div> : <div>
                    <Tree
                        checkable={treeCheckAble}
                        showIcon
                        style={{color: '#3F8EFF', fontSize: '12px'}}
                        onSelect={onSelect}
                        loadData={loadAsyncData}
                        treeData={treeList}
                        blockNode={true}
                        height={_height + 90}
                        className="tree"
                    />
                </div>
                }
                <div className="collapse-icon">
                  <LeftOutlined className="icon left-arrow" onClick={() => setIsExpand(false)}/>
                  <RightOutlined className="icon right-arrow" onClick={() => setIsExpand(true)}/>
                </div>
              </div>
            </div>
            <div className="file-content">
              {/* <OfficeChild showAdvance={() => setDrawVisible(true)}/> */}
              {/* 上层table */}
              <div className={showMenus ? 'office-child show-menus' : 'office-child'}>
                <div className="condition">
                  {
                    searchForm && <><Button size="small" type="primary" onClick={() => setDrawVisible(true)} style={{marginRight: '10px'}}>高级搜索</Button></>
                  }
                  {   // 右侧表格搜索框
                    tableSearch &&  <><Search size="small" placeholder="请输入搜索内容" style={{ width: 300, marginRight: 10 }} onSearch={(val) => onTableSearch(val)}/></>
                  }
                  {enableAdd && <EpsAddButton column={props.source} title={props.title.name} service={props.mainTableService} store={tableStore} customForm={customForm}></EpsAddButton>}
                  {
                    // 自定义功能按钮
                    customAction && customAction(tableStore)
                  }
                  <Tab shape="wrapped" excessMode="slide" className="add-btn-tab">
                    {
                      addBtnArr.map((item, index) => (
                        <Tab.Item
                          title={
                            <div className="add-btn">{item.title}
                              {editFlag ? <Icon type="delete-filling" className="del-icon" onClick={() => delBtn(item)}/> : ''}
                            </div>
                          }
                          key={item.title} onClick={() => item.onClick(selectedRowKeys, tableStore, checkedRows)}>
                          {item.title}
                        </Tab.Item>
                      ))
                    }
                  </Tab>
                  {
                    menuProp &&
                    <div className="menu-right" style={{display: 'block'}} onClick={() => setShowMenus(true)}><UnorderedListOutlined className="more"/>菜单</div>
                  }
                </div>
              <div className="table-container">
                <Table pagination={{
                  showQuickJumper: true,
                  showSizeChanger: true,
                  defaultCurrent: mainPage,
                  defaultPageSize: mainSize,
                  pageSize: mainSize,
                  current: mainPage,
                  size: 'small',
                  showTotal: (total, range) => `共 ${total} 条数据`,
                  onChange: onPageSizeChange,
                  total: mainTotal}}
                  bordered={true}
                  rowSelection={rowSelection}
                  columns={columns} loading={mainLoading} dataSource={mainTableList}  className="common-table"
                  scroll={{y: _height/2 - 40,x: 1800}}
                  onRow={record => {
                    return {
                      onClick: event => {
                        setClickRowData(record)
                      }, // 点击行
                    };
                  }}
                  />
              </div>
                <div className="menu-collapse">
                  <div className="group">
                      {
                      menuProp && menuProp.map((item, index) => (
                        <>
                          <li className="item" key={index} style={{backgroundColor: item.color}}>
                          <img style={{width: '28px'}}  src={require(`../../../../styles/assets/img/${item.icon}.png`)} onClick={() => item.onClick(selectedRowKeys, tableStore, checkedRows)}/>
                          <span>{item.title}</span>
                          <div className="shadow"></div>
                          {editFlag ? <img src={require('../../../../styles/assets/img/icon_add_border.png')} className="add-icon" onClick={() => clickAdd(item)}/> : ''}
                          </li>
                        </>
                      ))
                      }
                  </div>
                  <div className="btns">
                      <Button size="small" type="primary" onClick={() => setEditFlag(true)}>编 辑</Button>
                      <Button size="small" style={{margin: '0 20px'}} onClick={() => setEditFlag(false)}>重 置</Button>
                      <Button size="small" onClick={() => {setShowMenus(false), setEditFlag(false)}}>取 消</Button>
                  </div>
                </div>
                <span className="cover"></span>
              </div>
              {/* 上层table 结束 */}
              <hr style={{marginBottom: 5}}></hr>
              {/* 下层table 开始 */}
              <div className={showMenus ? 'office-child show-menus' : 'office-child'}>
                <div className="condition">
                  {
                    searchForm && <><Button size="small" type="primary" onClick={() => setDrawVisible(true)} style={{marginRight: '10px'}}>高级搜索</Button></>
                  }
                  {   // 右侧表格搜索框
                    tableSearch &&  <><Search size="small" placeholder="请输入搜索内容" style={{ width: 300, marginRight: 10 }} onSearch={(val) => onTableSearch(val)}/></>
                  }
                  {enableAdd && <EpsAddButton column={props.source} title={props.title.name} service={props.childTableService} store={tableStore} customForm={customForm}></EpsAddButton>}
                  {
                    // 自定义功能按钮
                    customAction && customAction(tableStore)
                  }
                  <Tab shape="wrapped" excessMode="slide" className="add-btn-tab">
                    {
                      addBtnArr.map((item, index) => (
                        <Tab.Item
                          title={
                            <div className="add-btn">{item.title}
                              {editFlag ? <Icon type="delete-filling" className="del-icon" onClick={() => delBtn(item)}/> : ''}
                            </div>
                          }
                          key={item.title} onClick={() => item.onClick(selectedRowKeys, tableStore, checkedRows)}>
                          {item.title}
                        </Tab.Item>
                      ))
                    }
                  </Tab>
                  {
                    menuProp &&
                    <div className="menu-right" style={{display: 'block'}} onClick={() => setShowMenus(true)}><UnorderedListOutlined className="more"/>菜单</div>
                  }
                </div>
              <div className="table-container">
                <Table pagination={{
                  showQuickJumper: true,
                  showSizeChanger: true,
                  defaultCurrent: childPage,
                  defaultPageSize: childSize,
                  pageSize: childSize,
                  current: childPage,
                  size: 'small',
                  showTotal: (total, range) => `共 ${total} 条数据`,
                  onChange: onPageSizeChange,
                  total: childTotal}} bordered={true} columns={childColumns} loading={childLoading} dataSource={childTableList}  className="common-table"
                  scroll={{y: _height/2 -41 ,x: 1800}}
                  style={{minHeight: `${_height/2 -41}px !important`}}
                  />
              </div>
                {/* <div className="menu-collapse">
                  <div className="group">
                      {
                      menuProp && menuProp.map((item, index) => (
                        <>
                          <li className="item" key={index} style={{backgroundColor: item.color}}>
                          <img style={{width: '28px'}}  src={require(`../../../../styles/assets/img/${item.icon}.png`)} onClick={() => item.onClick(selectedRowKeys, childTableStore, checkedRows)}/>
                          <span>{item.title}</span>
                          <div className="shadow"></div>
                          {editFlag ? <img src={require('../../../../styles/assets/img/icon_add_border.png')} className="add-icon" onClick={() => clickAdd(item)}/> : ''}
                          </li>
                        </>
                      ))
                      }
                  </div>
                  <div className="btns">
                      <Button size="small" type="primary" onClick={() => setEditFlag(true)}>编 辑</Button>
                      <Button size="small" style={{margin: '0 20px'}} onClick={() => setEditFlag(false)}>重 置</Button>
                      <Button size="small" onClick={() => {setShowMenus(false), setEditFlag(false)}}>取 消</Button>
                  </div>
                </div> */}
                <span className="cover"></span>
              </div>
              {/* 下层table 结束 */}
            </div>
          </div>
        {/* <AdvancedSearch drawVisible={drawVisible} changeVisible={() => setDrawVisible(false)} searchForm={searchForm} tableStore={tableStore}></AdvancedSearch> */}
      </div>
      </>
    );
}))

export default EpsPanel;
