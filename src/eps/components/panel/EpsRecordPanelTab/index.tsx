...
export interface RecordPanelProps extends EpsTableProps{
  tabs?: Array<RecordTabsProps>    // tab列表
  defaultTabs?: any;               // tab默认值
...
const EpsPanel = observer(forwardRef((props: RecordPanelProps, ref) => {
   ...
   //   props.onTabChange("", tableStore, checkedRows)}
    }
        ...
        if (enableDelete || enableEdit || actionWidth > 0) {
            columns.push({
              ...
              render: (text,record,index) => {
                ...
                return (
                <>
                  {customTableAction && customTableAction(text, rcd, index, tableStore)}
                  {enableEdit && <EpsEditButton  key={'edit_button_' + index} width={props.formWidth} column={props.source} title={props.title.name} data={rcd} store={tableStore} customForm={customForm}/>}
                  {enableDelete && <EpsDeleteButton  key={'delete_button_' +index} data={text} store={tableStore} />}
                </>
              )},
            });
          }
          ...
          <div className={'content hideExpand'}>
            <div className={showMenus ? 'right show-menus' : 'right'}>
              ...
              </div>
              {
                ...
                <Table columns={columns} dataSource={tableList} bordered scroll={{ x: 1600 }} pagination={{
              ...
              <Tabs defaultActiveKey={props.defaultTabs} onChange={(val) =>props.onTabChange(val, tableStore, checkedRows)} size="small" className="record-tabs">
                {
                  props.tabs && props.tabs?.map((item, index) => (
                    <TabPane tab={item.name} key={item.key}>
                        <item.comp {...props} store={tableStore} data={checkedRows} />
                    </TabPane>
                  ))
                }
              </Tabs>
