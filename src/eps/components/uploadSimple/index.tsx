...
const EpsUploadSimple = observer((props, refs) => {
  const ref = useRef();
  ...
  const initstore: IProps = useLocalObservable(() => ({
    ...
    async convert(val) {
      ...
      // const response=await fetch.post(`/api/eps/wdgl/attachdoc/wjzh`,this.params,{info, ...this.params,});
      // if (response.status === 200) {
      ...
      //  const zh = await fetch.post("/api/eps/wdgl/attachdoc/wjzh?files="+ aaaa);
      debugger;
      // if (zh.success == true) {
      //   ("转换成功");
      // } else {
  ...
  const onViewDocClick = async (val) => {
    if (val.length != 1) {
      ...
      if (response.data.message === 'Y') {
        // if(val[0].ext.toLowerCase()=="mp3" || val[0].ext.toLowerCase()=="mp4" ||  val[0].ext.toLowerCase()=="avi"){
        //   var pdfurl = encodeURIComponent('http://' + location.hostname + ':' + location.port+'/api/eps/wdgl/attachdoc/download?fileid=' +val[0].fileid + "&daktmid=" + props.daktmid
        ...
        //     + "&downlx=01&ext=.mp4"+"&tmzt=" + props.tmzt);
        //   window.open('/api/eps/control/main/app/lib/media/viewer.html?downfile='+0+'&printfile='+0+'&file='+pdfurl+'&filename='+val[0].filename);
        //   return;
        // }

        // var fileUrl = 'http://' + location.hostname + ':' + location.port+'/api/eps/wdgl/attachdoc/download?fileid='
        // + val[0].fileid+ '&grpid=' + initstore.sjgrpid + '&doctbl=' + props.uploadProp.doctbl
        // + '&daktmid=' + props.daktmid + "&ext=" + val[0].ext+ '&grptbl= '
        // + props.uploadProp.grptbl + '&atdw= ' + props.uploadProp.dw + '&umid='
        // + props.uploadProp.umId + '&mkbh=null&downlx=02&issy=f'
        // + "&dakid=" +  props.uploadProp.dakid + "&tmzt=" + props.tmzt+ '&fullfilename='+val[0].filename+"."+val[0].ext;
        // const previewUrl = fileUrl + '&fullfilename='+val[0].filename+"."+val[0].ext;
        ...
        const fileUrl =
          ...
          ':' +
          location.port +
          '/api/eps/wdgl/attachdoc/downloadOnlyFile?fileid=' +
          val[0].fileid +
          '&grpid=' +
          initstore.sjgrpid +
          '&doctbl=' +
  ...
  const onOfdviewClick = async (val) => {
    if (val.length < 1) {
      ...
      let url = '/api/eps/wdgl/attachdoc/OFDToPdf';
      // const response=await fetch.get(url, { params: param });
      ...
      // var purl=encodeURIComponent("http://127.0.0.1:8089/web-reader/reader?file=" + response.data);
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return [
      <>
        ...
        {/* {enableOfdview && <Button type="primary"  icon={<ProfileOutlined />} style={{ marginRight: 10, fontSize: 12 }}  onClick={() => onOfdviewClick(ids)}>OFD文档查阅</Button>} */}
        {enableConvertFiles && (
          <Button
            type="primary"
        ...
        {/* {enableDown && <Button type="primary" icon={<DownloadOutlined />} style={{ marginRight: 10, fontSize: 12 }} onClick={() => onOfdownloadClick(ids)}>OFD下载</Button>} */}
      </>,
    ];
  };
...
export default EpsUploadSimple;
