...
import ApplyDetail from './Daxc9ModuleDetailNoToolbar';
import { EpsTableStore } from '@/eps/components/panel/EpsPanel3';
import ArchDetail from './Daxc9Detail';
import EpsModalButton from '@/eps/components/buttons/EpsModalButton';
import ArchModuleAllEditButton from './Daxc9ModuleAllEditButton';
...
const Daxc9Module = observer((props) => {
  ...
  const [xcdApply_visible, setXcdApplyVisible] = useState(false);
   ...
   * 查询条件按钮点击事件
   */
  const doSearchAction = async (formData, tableStore) => {
    const {
      datebe,
      dwmc,
      bm,
    ...
    await tableStore.findByKey(tableStore.key, 1, tableStore.size, params);
  };
  const ref = useRef();
  ...
  useEffect(() => {
    ...
    ApplyStore.getUserOptionByDALYF002();
  ...
  const [xcdApplyForm] = Form.useForm();
   ...
   * 点击新建按钮事件
   */
  const showXcdApply = async () => {
    ApplyStore.getUserOptionByDALYF002();
    setXcdApplyVisible(true);
  };
    ...
    return [
      <>
        ...
        {OptrightStore.hasRight(umid, 'SYS101') && (
          <Button type="primary" onClick={() => showXcdApply(record, rows)}>
            {' '}
            新建{' '}
          </Button>
        )}
    ...
    return (
      <>
        <Row gutter={20}>
          ...
          <Col span={span}>
            <Form.Item label="所属全宗:" name="ssjzmc">
              <Input allowClear disabled style={{ width: _width }} />
            </Form.Item>
          </Col>
   ...
   * 新增协查单申请
   */
  const onPut_XcdApply = async (value) => {
    setXcdApplyVisible(false);
    await NewDajyStore.addXcdApply(value);
    const store = ref.current?.getTableStore();
    await doSearchAction({}, store);
  };
  ...
  return (
    <>
      ...
      <Modal
        title={<span className="m-title">协查单申请</span>}
        visible={xcdApply_visible}
        onOk={() => {
          xcdApplyForm.validateFields().then((values) => {
            xcdApplyForm.resetFields();
            onPut_XcdApply(values);
          });
        }}
        onCancel={() => setXcdApplyVisible(false)}
        width="1000px"
        style={{ top: 50 }}
      >
        <div style={{ height: 450 }}>
          ...
          >
            <Row>
              ...
              <Col span={8}>
                {/* <div style={style}> */}
                <Form.Item label="所属卷宗:" name="ssjzid">
                  <TreeSelect
                    style={{ width: 200 }}
              ...
              <Col span={8}>
                ...
                <Form.Item
                  label="借阅天数:"
                  name="jyts"
                  initialValue={ApplyStore.pDALYF002}
                  rules={[{ required: true, message: '借阅天数不允许为空!' }]}
                >
                  <InputNumber style={{ width: 197 }} min={1} max={99} />
                </Form.Item>
                {/* </div> */}
              </Col>

              <Col span={24}>
                <Form.Item label="" name="opetionGroup" style={{ height: 40 }}>
                  <Checkbox.Group
                    key={defaultVals}
