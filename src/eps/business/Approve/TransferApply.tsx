...
import DapubStore from './DapubStorex';
...
const TransferApply = observer((props) => {
  ...
  const wfreturn = async (ids) => {
    if (ids.length === 0) {
      ...
      const reponse = await fetch.post(
        `/api/eps/control/main/gsyjjssqd/wfreturnyjSqd`,
        fd,
      );
      debugger
      if (reponse.status === 200) {
        if (reponse.data?.success) {
          message.success({ content: '已退回!' });
        }else{
          message.error({ type: 'error', content:reponse.data?.message });
        }
      }else{
         message.error({ type: 'error', content:reponse.data?.message });
      }
  ...
  const customActionjs = (store: EpsTableStore, ids: any[]) => {
    return [
      <>
        <Form
          onFinish={(formData) => doSearchAction(formData, store)}
        ...
        <Button type="primary" onClick={() => wfreturn(store.selectedRowKeys)}>退回移交流程</Button>
        {/* <Button type="primary" onClick={() => showSxjction()} >四性检测</Button>
