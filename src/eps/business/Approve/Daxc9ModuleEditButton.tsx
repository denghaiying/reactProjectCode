...
function Daxc9ModuleEditButton(props: IProps) {
  ...
  const showModal = () => {
    ...
    if (props.data.dzjy === 'Y') {
      dzjy = true;
    } else {
      dzjy = false;
    }
    if (props.data.stjy === 'Y') {
      stjy = true;
    } else {
      stjy = false;
    }
  ...
  const onPut_Xg = async (value) => {
    ...
    const res = await NewDajyStore.updateXcdmx(value);
...
export default Daxc9ModuleEditButton;
