...
import ArchModuleEditButton from './ArchModuleEditButton';
...
const Inspection = observer((props) => {
  ...
  const findByDataLx = (text, record, index) => {
    ...
    if (record && record.dzjy === 'Y' && props?.detailParams?.wpid !== 'ZZZZ') {
      res.push(
        props.approve && (
    ...
    //是否可以查看,目前有问题还需调整(加上判断后就无法获取 record)
    //if (new_record.mxck === 'Y') {
