...
function ArchModuleAllEditButton(props: IProps) {
  ...
  const showModal = () => {
    ...
    if (!props.data.jydmxid && !props.allSelect) {
  ...
  const onPut_Xg = async (value) => {
    ...
    const res = await NewDajyStore.updateJydmxs(value);
      ...
      <Modal
        ...
        onOk={() => {
          xgfkForm.validateFields().then((values) => {
            xgfkForm.resetFields();
            onPut_Xg(values).then(() => {
              props.clearTableRowClickmx();
            });
          });
        }}
...
export default ArchModuleAllEditButton;
