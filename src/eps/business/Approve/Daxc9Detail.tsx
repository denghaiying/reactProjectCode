...
import { EpsPanel, EpsTableStore } from '@/eps/components/panel/EpsPanelNoToolBar';
...
const Daxc9Detail = observer((props) => {
    const ref = useRef();
    ...
    const store = useLocalObservable(() => (
        {
            ...
            async initKtable() {
                ...
                const childKfields = await TableService.getKField({ dakid: childKtable.id, lx: archParams.tmzt, pg: "list" });
                runInAction(() => {
                    this.childKfields = childKfields;
                    store.childColumns = childKfields.filter(kfield => kfield["lbkj"] === "Y").map(kfield => ({
    ...
    useEffect(() => {
        ...
        if (storeTable && store.childParams.bmc && storeTable.findByKey) {
            storeTable.findByKey('', 1, storeTable.size, { ...store.childParams, fid:props.recordId});
        }
    }, [store.childParams]);
