...
class DwService extends BaseService implements IEpsService, ITreeService {
  ...
  findTree(key: string = '') {
    ...
    return new Promise((resolve, reject) => {
      if (
        SysStore.getCurrentUser().golbalrole === 'SYSROLE01' ||
        SysStore.getCurrentUser().golbalrole === 'SYSROLE02'
      ) {
        return super
          .get({ url: address + `/queryForList_e9_superUser`, params })
          .then((res) => {
            if (res.status > 300) {
              return reject(res);
            }
        ...
        return super
          .get({
            url:
              address +
              `/queryForListByYhid_ReactTree?yhid=${
