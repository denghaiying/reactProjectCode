 ...
 *      新增代码
 */
const YjspSelectDailogVisiable = observer(props => {
  ...
  useEffect(() => {
    ...
    YjspSelStore.queryForList({ zt: 'I', yjrid: userinfo && userinfo.id, dakid: params.dakid });
    YjspSelStore.setSaveParams("title", `${params.dakmc}-${userinfo && userinfo.orgmc}-${curdate.format("YYYY")}${curdate.format("MM")}${curdate.format("DD")}`);
    setDlgVisible(true);
  }, [params]);
  useEffect(() => {
    ...
    YjspSelStore.addTm(keys).then(res => {
      if (res) {
        if (res.success) {
          ...
          IceNotification.info({ message: formatMessage({ id: 'e9.info.info' }), description: "加入移交申请单失败！\r\n" + res.message });
        }
...
export default injectIntl(YjspSelectDailogVisiable);
