 ...
 * 获取当前时间
 */
const getDate = moment().format('YYYYMMDDHHmmss');

const Dzwjzxyjzlyjjs = observer((props) => {
  const store = useLocalStore(() => ({
    ...
    async initSj() {
      const resBanner = await zlyjjsService.qureydajssytj({ztlx:2});
    ...
    if (timer == null) {
      timer = setInterval(() => {
        ...
        if (ZlyjjsStore.progressValue === 20) {
          ...
          ZlyjjsStore.xduoJc(store.recordsid,"DZWJJC"+moment().format('YYYYMMDDHHmmss')).then(() => {
            clearInterval(timer);
          });
  ...
  const onOpenpdf = async (val) => {
    if (val.jczt === 'Y') {
      const response =await fetch.post("/api/api/sxjcjgz/dcreatePdf?jcbc="+val.jcbch);
  ...
  const doDownloadAction = (record) => {
    if (record.gsyjsqd_wpid === 'ZZZZ') {
      ...
      ZlyjjsStore.packEEP_2022(params);
    } else {
      message.warning({
        type: 'warning',
        content: '审批未结束无法下载！',
