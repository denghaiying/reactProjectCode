...
const ArchDetail = observer((props) => {
    ...
    const store = useLocalStore(() => (
        {
            ...
            async initKtable() {
                ...
                runInAction(() => {
                    ...
                    this.childColumns = childKfields.filter(kfield => kfield["lbkj"] === "Y").map(kfield => ({
    ...
    useEffect(() => {
        ...
        if (storeTable && props.recordId && storeTable.findByKey) {
            debugger
            storeTable.findByKey('', 1, storeTable.size, { fid: props.recordId, ...props.archParams });
        }
    }, [props.recordId]);
    ...
    return (
        <>
            <EpsPanel title={title}                    // 组件标题，必填
                ...
                ref={ref}                                // 获取组件实例，选填
                formWidth={500}
                initParams={{bmc:store.childKtable.bmc,tmzt:archParams.tmzt}}
