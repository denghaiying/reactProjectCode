  ...
  const doRunAction=(code)=>{
    ...
    if(code=="直接开放"){
     ...
     if (!DapubStore.selectRowRecords || DapubStore.selectRowRecords.length < 1) {
      ...
      SelectDialStore.genSQD(selectDialogParams,true,(res)=>{
        IceNotification.info({ message: "提交成功", description: "提交成功" });
          ...
          <div className="group">
            {
              props.menuArr.map((item, index) => (
                <li className="item" onClick={()=>doRunAction(item)} key={index} style={{backgroundColor: colorArr[index]}}>
