...
import DwNomarlTableLayout from '@/eps/business/DwNomarlTableLayout';
...
const Yh = observer((props) => {
  ...
  const [yhmjdisabled, setYhmjdisabled] = useState(true);

  const [qtdisabled, setQtdisablede] = useState(true);
  ...
  const tableProp: ITable = {
    ...
    disableEdit:
      SysStore.getCurrentUser().golbalrole == 'SYSROLE02' ? false : false,
    disableDelete:
      SysStore.getCurrentUser().golbalrole == 'SYSROLE02' ? true : false,
    ...
    onAddClick: (form) => {
      setYhmjdisabled(false);
      setQtdisablede(false);
    },
    onEditClick: (form, data) => {
      if(data.tyrq){
        data.tyrq=moment(data.tyrq);
      }
      console.log(SysStore.getCurrentUser().golbalrole);
      if(SysStore.getCurrentUser().golbalrole == 'SYSROLE01'){
        setQtdisablede(false);
        setYhmjdisabled(true);
      }else if(SysStore.getCurrentUser().golbalrole == 'SYSROLE02'){
        ...
        <Row gutter={20}>
          ...
          <Col span={span}>
            <Form.Item label="性别" name="xb">
              <Select  disabled={qtdisabled} style={{ width: _width }} className="ant-select">
          ...
          <Col span={span}>
            <Form.Item label="停用日期" name="tyrq">
              <DatePicker placeholder="停用日期" style={{ width: _width }} disabled={qtdisabled}/>
              {/*
...
*/}
          ...
          <Col span={span} hidden={kfhjurlShow()}>
            <Form.Item label="库房环境URL" name="kfhjurl">
              <input className="ant-input" style={{ width: _width }}  disabled={qtdisabled}/>
            </Form.Item>
          </Col>
  ...
  const getCustomAction = (store: EpsTableStore, ids: any[]) => {
    let res: Array<any> = [];
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return getCustomAction(store, ids);
  };
  ...
  const getCustomTableAction = (text, record, index, store) => {
    ...
    if (SysStore.currentUser.golbalrole == 'SYSROLE01') {
      ...
      res.push(
        <YhUpPwd text={text} record={record} index={index} store={store} />,
      );
      res.push(
        <RoleYh text={text} record={record} index={index} store={store} />,
      );
    } else {
      res.push(UnLock(text, record, index, store));
      res.push(RestPwd(text, record, index, store));
