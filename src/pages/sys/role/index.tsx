...
import copyRoleqx from './copyRoleqx';
...
const Role = observer((props) => {
  ...
  const customTableAction = (text, record, index, store) => {
    return (
      <>
        {[
          <CopyRole text={text} record={record} index={index} store={store} />,
    ...
    return [
      <>
        ...
        <EpsModalButton
          ...
          beforeOpen={(ids) => {
            if (ids.roleid == undefined) {
              message.error('操作失败,请选择一行数据');
              return false;
            } else {
              childStore.roleid = ids.roleid;
              return true;
            }
          }}
          params={{ roleid: ids[0]?.id, dwid: RoleStore.dw_id }}
          icon={<SettingOutlined />}
        />
