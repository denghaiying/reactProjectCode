...
const Lcdzlk = observer((props) => {
  // const [dakparams, setDakparams] = useState({});
  const [rowId, setRowId] = useState();
  const [mbDakRowId, setMbDakRowId] = useState();
  const [saveYw, setSaveYw] = useState('Y');
  ...
  const dakSaveDoAutoDy = () => {
    ...
    while (i < num && i < mbnum) {
      ...
      for (let j = i; j < mbnum; j++) {
        ...
        if (LcdzlkStore.dakzlxdata[i].mc == record.mc) {
          if (j != i) {
            ...
            // LcdzlkStore.mbkzzlxdata.splice(i, 1);
            // LcdzlkStore.mbkzzlxdata.splice(i, 0, record);
            // const newmbdakzlxs = JSON.parse(JSON.stringify(LcdzlkStore.mbkzzlxdata));
            // LcdzlkStore.mbkzzlxdata = newmbdakzlxs;
          }
          flag = true;
          break;
      ...
      if (!flag) {
        ...
        // var record = LcdzlkStore.dakzlxdata[i];
        // LcdzlkStore.dakzlxdata.splice(i, 1);
        // LcdzlkStore.dakzlxdata.push(record);
        // const newdakzlxs = JSON.parse(JSON.stringify(LcdzlkStore.dakzlxdata));
        // LcdzlkStore.dakzlxdata = newdakzlxs;
        num--;
      } else {
        i++;
