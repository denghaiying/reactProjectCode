...
const WjzhKkjs = (props) => {
  //const tmzt=3
  console.log('WjzhKkjsprops', props);
  ...
  const onSelect = (selectedKeys, info) => {
    ...
    let wzlk = 'N';
    debugger;
    if (others.mc.indexOf('卷内') > 0) {
      if (others.mc.indexOf('未整理') >= 0) {
        wzlk = 'Y';
      } else {
        wzlk = 'N';
      }
    ...
    const params: ArchParams = {
      umid: props.jdumid,
      umname: `${info.node.mc}【${props.location.query.name || props.jdname}】`,
      bmc: others.mbc,
      dakid: others.id,
      lx: others.lx,
      wzlk: wzlk,
  ...
  const ontaleSelect = async (record) => {
    ...
    const params: ArchParams = {
      umid: props.jdumid,
      umname: `${dak.data.mc}【${props.location.query.name || props.jdname}】`,
      bmc: dak.data.mbc,
      dakid: dakid,
      lx: dak.data.lx,
      wzlk: 'N',
      ...
      tmzt: props.tmzt,
      key: Store.key,
      path: `/runRfunc/archManage/${dakid}/${props.tmzt}`,
    };
    runFunc(params);
    // history.push({
    //   pathname: `/eps/dagl/archManage/${dakid}/${props.tmzt}`,
    //   query: params,
    // });
  };
    ...
    <Observer>
      {() => (
        <div className="oa-manage">
          <div className="title">
            {props.location.query.name || props.jdname}
          </div>
