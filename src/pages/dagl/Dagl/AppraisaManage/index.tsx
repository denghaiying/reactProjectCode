...
const ArchManage = observer((props) => {
  ...
  let archParams: ArchParams = props.archParams;
  // 权限类型,通过参数控制档案库界面各组件（如菜单）显示. modal:弹窗类型不显示菜单按钮编辑区域
  const authType = archParams?.params?.authType;
  // 动态按钮 通过传递参数显示按钮，不从后台获取按钮
  const dynamicButtons = archParams?.params?.dynamicButtons;
  if (props.fid) {
    archParams.fid = props.fid;
  }
  ...
  const archStore: ArchStoreType = useLocalObservable(() => ({
    ...
    uploadProp: {
      ...
      disableMediaTwo: false,
      disableMediaSpilit: false,
      disableMediaConcat: false,
    ...
    fjsource: [
      ...
      },
      {
        ...
        formType: EpsFormType.Input,
        render: (text, record, index) => {
          let xfglist = archStore.fjmjlist;
          let aa = xfglist.filter((item) => {
            return item.value === text;
          });
          return aa[0]?.label;
    ...
    async refreshPage(filter) {
      ...
      storeTable.findByKey('', 1, storeTable.size, {
        fid: props.fid,
        key: filter && filter.join(' '),
        ...props.archParams,
      });
    },
    hasMenuAuth(authType: string) {
      return authType != 'modal';
    },
    doArchactionHandle(ids, store, records, item) {
      if (
        ArchCommon[item.optcode] &&
        ArchCommon[item.optcode].selectRowsCheck
      ) {
        if (ArchCommon[item.optcode].selectMulitMode && ids.length < 1) {
          showMessage(
            ArchCommon[item.optcode].selectRowsCheckWarMessage,
            'warn',
          );
          return;
        }
      ...
      if (ArchCommon[item.optcode] && ArchCommon[item.optcode].showPopconfirm) {
        confirm({
          ...
          onOk: () =>
            archStore.doArchAction(item, ids, store, records, this.ktable),
      ...
      return dakopt.map((item) => {
        return {
          ...
          onClick: (ids, store, records) => {
            this.doArchactionHandle(ids, store, records, item);
          },
          params: {
            tmzt: this.tmzt,
    ...
    initSysOpertion(archParams: ArchParams, bmc: string, dakqx) {
      ...
      if (dakqx != undefined && dakqx != null && dakqx != '') {
        ...
        if (dakqx.indexOf('SYS201') > 0) {
          this.enablPrint = true;
        }
        /**附件权限 */
    ...
    async initArchInfo(archParams: ArchParams, ktable) {
      ...
      const { tmzt } = archParams;

      if (archStore.hasMenuAuth(authType)) {
        ...
        runInAction(() => {
          if (tmzt === '4' || tmzt === 4) {
            archStore.uploadtableProp['disableEdit'] = true;
            archStore.uploadtableProp['disableDelete'] = true;
          }
          if (tmzt === '12' || tmzt === 12) {
            if (archParams.umid === 'DALY025') {
              archStore.tableProp['disableEdit'] = false;
            }
          }

          archStore.ktable = ktable;
          archStore.advSearchColumns = mbcx || [];

          if (tmzt == '8') {
            archStore.dakCartCount = dakCartCount;
          }
          this.tmzt = archParams.tmzt;
          this.dakid = archParams.dakid;
        ...
        runInAction(() => {
          ...
          // 菜单按钮
          this.menuButton = this.getMenuButton(dynamicButtons);
          // 系统按钮权限
    ...
    // 初始化列
    async initColumns(archParams: ArchParams, ktable, treevalue) {
      console.log('初始化列initColumns');
      if (!ktable.bmc) {
        return;
      }
        ...
        this.stflid =
          treevalue == null
            ? archStore.stflid === ''
              ...
              : archStore.stflid
            : treevalue.split(';')[1];
        kfieldsparams['stflid'] = this.stflid;
      }
      const kfields = await TableService.getKField(kfieldsparams);
      runInAction(() => {
        this.ktable = ktable;
        ...
        this.columns = kfields
          ...
          .map((kfield) => {
            let result;
            return {
              width: kfield['mlkd'] * 1.4,
              code: kfield['mc'].toLowerCase(),
              title: kfield['ms'],
              align: 'center',
              ellipsis: true,
              //  render:!kfield.gsh?undefined:(value,record)=>{
              //   return moment(value).format(kfield.gsh);
              // }
            };
          });
      });
    ...
    async initAuth(archParams: ArchParams, ktable) {
      ...
      const ywpsql = await TableService.getSjqxSql({
        pcode: 'B',
        dakid: this.dakid,
        tmzt: this.tmzt,
        yhid: SysStore.getCurrentUser().id,
      ...
      runInAction(() => {
        ...
        archParams.ywpsql = sqlencode(getParamSql(ywpsql, ktable.bmc));
        archParams.inited = 'Y';
        archParams.sfdyw = ktable.sfdyw == 'Y';
        this.sfdyw = ktable.sfdyw == 'Y';
        console.log('psql', archParams.psql);
        //  this.dakopt = dakOptAll;
        this.archParams = archParams;
        console.log('storeParams', this.archParams);
        // 系统按钮权限
        // 表格菜单
    ...
    async menuLoad() {
      ...
      if (archStore.hasMenuAuth(authType)) {
        if (tmzt && tmzt > 0) {
          dakopt = await dakoptService.findAll({
            tmzt,
            isadd: 'N',
            optlx: '1',
            dakid: archParams.dakid,
          ...
          dakyhOpt = await dakoptService.findAll({
            isadd: 'Y',
            daklx: archParams.lx,
            optlx: '1',
            wzlk: archParams.wzlk ? archParams.wzlk : undefined,
    ...
    doArchAction: async (
      ...
      if (opt && opt.optcode) {
        ...
        runInAction(() => {
          ...
          archStore.disableRefresh = archModalInfo.disableRefresh;
          // 全屏弹窗
          if (archModalInfo.fullModal) {
            archStore.modalHeight = document.body.clientHeight - 72;
            archStore.modalWidth = '100%';
            archStore.modalStyles = {
              top: '0px',
              margin: '0px',
              maxWidth: '100%',
            };
          } else if (archModalInfo.modalStyles) {
            archStore.modalStyles = {};
            archStore.modalStyles = archModalInfo.modalStyles;
          } else {
            archStore.modalStyles = {};
          }
          archStore.selectRecords = records;
          ...
          // onclose
          if (archModalInfo.onClose) {
            archStore.onClose = archModalInfo.onClose;
          }
        });
      }
    },
      ...
      if (ids.length <= 0) {
        ...
        if (dk === 1) {
          return {
            ...
            height: '580px',
            showFoot: false,
            url: '/eps/business/Approve/DzwjzxSelectyjzlDailog',
            title: '整理移交',
            extendParams: {
              ids: ids.join(','),
              spUrl: 'dzwjzxsqd',
    ...
    DAK0010: (
      ...
      if (archParams.gcid) {
        filter['fid'] = archParams.gcid;
      }
      if (archStore.fid) {
        filter['fid'] = archParams.fid;
      }
      return {
        url: `/api/eps/control/main/dagl/pldr`,
        params: filter,
        title: '批量导入',
        filter,
        width: 1280,
    ...
    // SIP导入
     DAK0023: async (
      ...
      let filter = {
        ...
        yhmc: dakoptService.yhmc,
        cjdw: dakoptService.dwid,
        dwmc: SysStore.getCurrentCmp().mc,
        daklx: archStore.ktable?.daklx,
        mbid: archStore.ktable?.mbid,
        dakbmc: params.bmc,
        dakmc: params.mc,
        yjdwbh: SysStore.getCurrentCmp().dwbh,
        mkxx: '档案管理/档案管理/[' + params.mc + ']',
    ...
    DAK0052: (
      ...
      return {
        url: `/api/eps/control/main/dagl/zdyfz`,
        params: filter,
        title: '自定义分组设置',
        filter,
        width: 900,
    ...
    DAK0014: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakhj`,
        params: filter,
        title: '合到新卷',
        filter,
        width: 700,
    ...
    DAK0021: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakDakcc`,
        params: filter,
        title: '档案查重',
        filter,
        width: 1200,
    ...
    DAK0022: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakDhDhcx`,
        params: filter,
        title: '断号查找',
        filter,
        width: 1300,
    ...
    DAK0063: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakzhpx`,
        params: filter,
        title: '综合排序设置',
        filter,
        width: 900,
    ...
    DAK0068: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakplzj`,
        params: filter,
        title: '批量组件设置',
        filter,
        width: 900,
    ...
    //   };
    //   return {
    //     url: `/api/eps/control/main/hgl/hgl_da`,
    //     params: filter,
    //     title: '装盒',
    //     filter,
    //     width: 985,
    ...
    //   return {
    //     url: `/api/eps/control/main/hgl/openDak`,
    //     params: filter,
    //     title,
    //     filter,
    //     width: 1200,
    ...
    async DAK0201(
      ...
      if (ids.length < 1) {
        ...
        await dakoptService.saveTmid(tmids);
      ...
      localStorage.setItem('mbid', archStore.ktable?.mbid);
    ...
    DAK0055: (
      ...
      return {
        url: `/api/eps/control/main/dagl/pldcgl`,
        params: filter,
        title: '定时批量导出',
        filter,
        width: 600,
    ...
    DAK0051: (
      ...
      // return { disableModal: true };
      const url =
        '/api/eps/control/main/dagl/downloadZip1?downlx=02&dakid' +
        dakid +
        '&grptbl=' +
        grptbl +
        '&doctbl=' +
    ...
    DAK0056: (
      ...
      const url =
        '/api/eps/control/main/pldcgl/batchExportExcel?page=1&start=0&limit=100&tmzt=' +
        params.tmzt +
        '&mbid=' +
        archStore.ktable?.mbid +
        '&dakid=' +
    ...
    DAK0048: (
      ...
      return {
        url: `/api/eps/control/main/dagl/cjcakck`,
        params: filter,
        title: '参见档案',
        filter,
        width: 800,
    ...
    DAK0111: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakZl`,
        params: filter,
        title: '档案整理',
        filter,
        width: 800,
    ...
    DAK0017: (
      ...
      return {
        url: `/api/eps/control/main/dagl/daksaveas`,
        params: filter,
        title: '另存到其它档案库',
        filter,
        width: 1080,
    ...
    DAK0028: (
      ...
      return {
        url: `/api/eps/control/main/dagl/jndakZl`,
        params: filter,
        title: '调整卷内顺序号',
        filter,
        width: 1080,
    ...
    DAK0029: (
      ...
      return {
        url: `/api/eps/control/main/dagl/ywcmm`,
        params: filter,
        title: '原文重命名',
        filter,
        width: 900,
    ...
    async DAK0064(
       ...
       */
      return {
        url: `/api/eps/control/main/dagl/logLine`,
        params: filter,
        title: '日志',
        filter,
        width: 1280,
    ...
    DAK0103: (
       ...
       */
      return {
        url: `/api/eps/control/main/dagl/bykhbyytm`,
        params: filter,
        title: '合并到已有条目',
        filter,
        width: 1280,
    ...
    DAK0115: (
      ...
      return {
        ...
        height: '300px',
        showFoot: false,
        url: '/eps/business/dagl/gpdb/index',
        title: '光盘打包',
        extendParams: {
          dataParams: filter,
          // ids: ids.join(','),
    ...
    DAK0122: (
      ...
      debugger;
      console.log('ids: ids.toString()____ids: ids.toString()', ids);
      return {
        url: `/api/eps/control/main/dagl/dakhyyj`,
        params: filter,
        title: '合已有卷',
        filter,
        width: 1300,
    ...
    //   };
    //   return {
    //     url: `/api/eps/control/main/dagl/xdaglDayj?${qs.stringify(filter)}`,
    //     params: filter,
    //     title: '档案移交',
    //     filter,
    //     width: 600,
    ...
    DAK0080: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakbysaveas`,
        params: filter,
        title: '另存到编研档案库',
        filter,
        width: 950,
    ...
    DAK0071: (
      ...
      return {
        url: `/api/eps/control/main/daly/plcx`,
        params: filter,
        title: '批量查询',
        filter,
        width: 1100,
    ...
    DAK0086: async (
      ...
      if (res.success) {
        ...
        window.top.RightStore.queryAllWdscCount();
      } else {
        message.error('操作失败！原因：\r\n' + res.message);
      }
      return { disableModal: true };
    ...
    DAK0026: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakPlgl`,
        params: filter,
        title: '批量自动关联关系设置',
        filter,
        width: 960,
    ...
    DAK0013: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakzyyj`,
        params: filter,
        title: '组已有卷',
        filter,
        width: 900,
    ...
    DAK0037: (
      ...
      return {
        url: `/api/eps/control/main/dagl/dakyj`,
        params: filter,
        title: '移件',
        filter,
        width: 800,
    ...
    async DAK0121(
      ...
      archStore.refresh(filter);
      message.success({ type: 'success', content: '查询成功' });
      return { disableModal: true };
    },
    // 编研文档
    async DAK00105(
    ...
    async DAK0146(
      ...
      if (ids.length <= 0) {
        ...
        return {
          ...
          height: '180px',
          showFoot: false,
          url: '/eps/business/Approve/SelectDialogDivQxts',
          title: '期限提升',
          extendParams: {
            ids: ids.join(','),
            spUrl: 'qxts',
    ...
    async DAK0147(
      ...
      return {
        ...
        url: '/eps/business/dagl/searchFilter',
        title: '关键词查询',
        extendParams: {
          ids: ids.join(','),
          spUrl: 'qxts',
    ...
    async DY001(
      ...
      if (ids.length <= 0) {
        ...
        const res = await TableService.addDetail({
          id: archParams.kfjdid,
          ids: ids.join(','),
          kfjd: '审批中',
        });
        // props.callback(res);
        if (res.success) {
          showMessage('调档成功', 'success');
          if (props.callback) {
            props.callback(res);
          }
        } else {
          showMessage(`调档失败:${res.message}`, 'success');
        }
        return { disableModal: true };
      }
    },
    ...
    doEdit: (modalType, record, index, store) => {
      runInAction(() => {
        ...
        let value = '';
        if (archStore.sfdyw) {
          if (archStore.tablekey === '') {
            message.error('请选择左边树！');
            return;
          }
          value = archStore.tablekey.split(';')[0].split('#value#')[1];
        }
        if (modalType == 'add') {
          archStore.getfjGuid();
          archStore.editRecord = archStore.sfdyw ? { flh: value } : {};
          archStore.modalType = '';
        } else {
    ...
    // }
    // archAction end
    openProjectArch: async (modalType, record, index, store) => {
      var dakid = record.dakid;
      const queryParams = archParams;
      const dak = await TableService.queryKTable({ fid: dakid });
      const params: ArchParams = {
        //  umid: props.jdumid,
        umname: `${dak.mc}【${queryParams.ztmc}】`,
        bmc: dak.bmc,
        dakid: dak.id,
        gcid: record.id,
        ajmc: `项目号：${record.xmh}`,
        path: `/runRfunc/archManage/${dak.id}/${archParams.tmzt}`,
        lx: dak.daklx,
        wzlk: 'N',
        type: 'K',
        mc: dak.mc,
        // title: dak.bh + '|' + dak.mc,
        tmzt: archParams.tmzt,
        //  key: `${dak.id}_${archParams.tmzt}`,
      };
      props.openProjectArch(params);
    },
    //另存到资料库
    DAK0151: (
      optcode: string,
  ...
  //长期保存档案库数据恢复
  var newCqbcrefresh = async (param) => {
    const cqbcrepson = await TableService.newcqbcDataRecovery(param);
    if (!cqbcrepson.success) {
      message.warning({
        type: 'warning',
        content: '数据恢复失败' + cqbcrepson.message,
  ...
  const refreshPage = async () => {
    archStore.refreshPage();
  };

  const refreshPagebmtz = async () => {
    archStore.refreshPagegsbmtz();
  };
  ...
  const deleteToHsz = (ids, store) => {
    ...
    //您所选择的条目中，有其他人员录入的信息，不能删除。（参数配置）
    const findParamsValueById = async () => {
      const res = await fetch.get(
        `/api/eps/control/main/params/queryParamsValueById?pcode=DAGLF014`,
      );
  ...
  const customAction = (store: EpsTableStore, ids: any[]) => {
    return [
      <>
        ...
        {/*{archStore.hasMenuAuth(authType) && (*/}
      ...
      return (
        <>
          ...
          {props.isProject && (
            <Tooltip title="工程案卷">
              <Button
                ...
                onClick={() =>
                  archStore.openProjectArch('edit', record, index, store)
                }
              />
            </Tooltip>
          )}
          {archStore.enablEdit && (
            <Tooltip title="修改">
              <a
                style={{ width: 22, margin: '0 2px', color: '#0070cc' }}
  ...
  // 监听fid刷新
  useEffect(() => {
    console.log('props.fid+props.fid', props.fid);
  ...
  const onSave = (values, modalType) => {
    ...
    if (archParams.gcid) {
      values.fid = archParams.gcid;
    }
    if (archParams.sfdyw) {
      values.stflid = archStore.stflid;
    }
    if (modalType == 'add') {
      const params = {
        dakid: archStore.dakid,
      ...
      storeTable
        ...
        .then((res) => {
          ...
          showMessage('数据新增成功', 'success');
          storeTable.findByKey(archStore.tablekey, 1, storeTable.size, {
            fid: props.fid,
            ...props.archParams,
          });
        })
  ...
  return (
    <div style={{ height: '100%' }}>
      <EpsPanel
        ...
        onStflChange={(treevalue) => {
          archStore.tablekey = treevalue;
          archStore.initColumns(archParams, props.ktable, treevalue);
        }}
      ></EpsPanel>

      <ArchMenuAction
        params={archStore.archParams}
        afterClose={archStore.afterModalClode}
        modalVisit={archStore.modalVisit}
        drawerVisit={archStore.drawerVisit}
        archStore={archStore}
        modalStyles={archStore.modalStyles}
        onClose={archStore.onClose}
        extendParams={archStore.extendParams}
        opt={archStore.menuActionItem}
      />

      {archParams.tmzt == 8 && archStore.hasMenuAuth(authType) && (
        <DrawerAction
          //   params={archStore.archParams}
