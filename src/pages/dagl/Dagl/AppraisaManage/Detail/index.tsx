...
const EpsRecordForm = observer((props: any) => {
  ...
  useEffect(() => {
    if (props.detailVisible && (props.sfdyw || store.formFileds.length < 1)) {
      form.resetFields();
    ...
    if (props.modalType === 'add') {
      ...
      runInAction(() => {
        ...
        store.xUpPloadprop = props.uploadprop;
        store.xUpPloadprop.disableUpload = true;
        store.xUpPloadprop.disableBigUpload = true;
        store.talbeparam = {
          wrkTbl: props.bmc,
          doctbl: props.bmc + '_FJ',
          grptbl: props.bmc + '_FJFZ',
  ...
  useEffect(() => {
    if (props.saveData?.id) {
      runInAction(() => {
        store.xUpPloadprop = props.uploadprop;
        store.xUpPloadprop.disableUpload = false;
        store.xUpPloadprop.disableBigUpload = false;
      ...
      .then((values) => {
        ...
        store.dateFiledFomrmat.forEach((kfield) => {
          if (values[kfield.mc.toLowerCase()]) {
            ...
            ].format(kfield.gsh ? kfield.gsh : 'YYYY-MM-DD hh:mm:ss');
          }
        });
