...
import './index.less'
import { Button, Drawer, Form, Row, Col, Input, Select, InputNumber, DatePicker, Divider, TreeSelect, message } from 'antd';
...
const EpsRecordForm = observer((props: DakFormProps) => {
  ...
  const store = useLocalObservable(() => ({
      ...
      detailService.getOptions(url, params).then((options) => {
        if (Array.isArray(options)) {
          const optionArr = options.map(item => {
            return { id: item.bh, title: item.mc, pid: "0", key: item.bh, isLeaf: true }
          });
          // runInAction(() => {
    ...
    async initFormKfileds(kfields, ktable) {
      ...
      me.bjkj = [];
      me.stflzd = [];
      const formKfields = kfields.filter(function (o) {
        return o.kpkj == 'Y'
      }).sort(function (a, b) {
        ...
        switch (kfield["kjlx"]) {
          case "C":
            ...
            if (kfield["cd"] && kfield["cd"] > 1) {
              if (kfield["sfxzcd"] == "Y") {
                ...
                rules.push({
                  len: kfield["xzcdws"],
                  message: kfield["ms"] + "：请输入" + kfield["xzcdws"] + "个字符"
                })
                ctrlcfg["vtype"] = "rangeLength:" + kfield["xzcdws"] + "," + kfield["xzcdws"];
                ctrlcfg["rangeLengthErrorText"] = kfield["ms"] + "：请输入" + kfield["xzcdws"] + "个字符";
              } else {
            ...
            if (kfield["cz"] == "Y") {
              ...
              if (kfield["czlx"] == "2") {
                ...
                if (kfield.gl == "Y") {
                  ...
                  ctrlcfg["onvaluechanged"] = "onstflvaluechanged('" + kfield.mc + "','" + kfield.glzlx + "')"
                }
                // stlfTemp.name = fmc;
                // me.stflzd.push(stlfTemp);
                // ctrlcfg["parentField"] = "fid";

                ctrlcfg["url"] = "/api/eps/control/main/stflmx/queryForPage?flid=" + kfield["czstfl"] +
                  '&mbid=' + ktable.mbid + '&dw=' + me.dwid + '&mbzlxmc=' + kfield["mc"];
                ...
                // ctrlcfg["parentField"] = "fid";
                debugger
                ctrlcfg["url"] = "/api/eps/control/main/org/queryDwOrgTree?dwid=" + me.dwid;
                me.loadOptions(fmc, ctrlcfg["url"], "", {});
                ...
                ctrlcfg["onvaluechanged"] = "onYhdwchange('" + kfield.mc + "','" + qzh[0].mc + "')"
                ...
                me.loadOptions(fmc, ctrlcfg["url"], kfield["czlx"], { fid: kfield["czsjzd"] });
          ...
          case "CB":
            ...
            if (kfield.sxid == "SX104") {
              ...
              ctrlcfg["onvaluechanged"] = "onSxrqchanged('" + kfield.mc + "','" + sxrqzd[0].mc + "','" + ctrlcfg["valueField"] + "')";
    ...
    async initKtable(props:DakFormProps) {
      debugger
      const ktable:KtableType=props.ktable;
      const tmzt:string=props.tmzt;
      const dakid:string=props.dakid;
      const kfields:any=props.kfields;
      if(ktable){
        runInAction(()=>{
          this.ktable=ktable;
      ...
      const resKtable= await TableService.queryKTable({dakid});
      const resKfield = await TableService.getKField({ dakid, lx: tmzt, pg: "list" });
      runInAction(()=>{
        this.ktable=resKtable;
        this.kfields=resKfield;
        this.dakid=dakid;
        this.tmzt=tmzt;
      });
    ...
    async inittableParam(ktable) {
      this.talbeparam = { 'wrkTbl': props.bmc, 'doctbl': props.bmc + "_FJ", 'grptbl': props.bmc + "_FJFZ", 'grpid': '' }
    },
    async getfjGuid() {
  ...
  useEffect(() => {
    ...
    if(props.detailVisible){
      store.initKtable(props);
    }
  }, [props.dakid,props.detailVisible])
  ...
  useEffect(() => {
    ...
    if (props.detailVisible) {
    form.resetFields()
    store.initFormKfileds(store.kfields, store.ktable)
    store.xUpPloadprop = props.uploadprop;
    store.inittableParam(props.dakid,props.tmid)
  ...
  if (props.modalType === 'add') {
    ...
    runInAction(() => {
      ...
      store.xUpPloadprop = props.uploadprop;
      store.talbeparam = { 'wrkTbl': props.bmc, 'doctbl': props.bmc + "_FJ", 'grptbl': props.bmc + "_FJFZ", 'grpid': '111' }
...
}, [store.tmzt,props.detailVisible])
  ...
  //         store.getfjGuid();
  //       }
  //       store.Uploadparam = { 'wrkTbl': props.bmc, 'docTbl': props.bmc + "_FJ", 'docGrpTbl': props.bmc + "_FJFZ", 'grpid': store.sjgrpid, 'daktmid': props.saveData?.id, 'tmzt': props.tmzt, 'dakid': props.ktable.id, 'atdw': currentCmp?.id, 'idvs': JSON.stringify({ id: props.saveData?.id }), 'mj': props.saveData?.mj };
  //       store.talbeparam = { 'wrkTbl': props.bmc, 'doctbl': props.bmc + "_FJ", 'grptbl': props.bmc + "_FJFZ", 'grpid': store.sjgrpid }
  //     });
  //   }
  // }, [props.saveData])
  // useEffect(() => {
  ...
  //         const dateValue = moment(props.editRecord[kfield.mc.toLowerCase()], kfield.gsh);
  ...
  //         }
  //         store.xUpPloadprop = props.uploadprop;
  //         store.Uploadparam = { 'wrkTbl': props.bmc, 'docTbl': props.bmc + "_FJ", 'docGrpTbl': props.bmc + "_FJFZ", 'grpid': store.sjgrpid, 'daktmid': record?.id, 'tmzt': props.tmzt, 'dakid': props.ktable.id, 'atdw': currentCmp?.id, 'idvs': JSON.stringify({ id: record?.id }), 'mj': record?.mj };
  //         store.talbeparam = { 'wrkTbl': props.bmc, 'doctbl': props.bmc + "_FJ", 'grptbl': props.bmc + "_FJFZ", 'grpid': store.sjgrpid }
  ...
  const data = []
  for (let i = 0; i < 50; i++) {
    let obj = { key: i + 1, order: i + 1, title: '测试标题' + i + 1, f_name: '文件一', f_type: 'pdf', f_size: '1.37M', trans: '' }
 ...
 this.miniType = {
  ...
  const createFormItem = (item) => {
    ...
    if (item.componentType == "C") {
      return <Col span={6}><Form.Item initialValue={getInitialValue(item)} label={item.fieldLabel} name={item.name}

        rules={item.rules}
        disabled={item.readonly == 'Y' ? true : false}><Input placeholder={item.fieldLabel} /></Form.Item></Col>
    } else if (item.comboxType == "Y") {

      return <Col span={6}><Form.Item initialValue={getInitialValue(item)} label={item.fieldLabel} name={item.name} required={item.require == 'Y' ? true : false} disabled={item.readonly == 'Y' ? true : false}>
        <TreeSelect placeholder={item.fieldLabel} treeData={store.cacheCombox[item.name]} loadData={store.loadOptions} />

      </Form.Item></Col>
    } else if (item.componentType == "D") {

      return <Col span={6}><Form.Item initialValue={moment(new Date(), item.format)} label={item.fieldLabel} name={item.name} required={item.require == 'Y' ? true : false} disabled={item.readonly == 'Y' ? true : false}>
      ...
      return <Col span={6}><Form.Item initialValue={getInitialValue(item)} required label={item.fieldLabel} name={item.name} required={item.require == 'Y' ? true : false} disabled={item.readonly == 'Y' ? true : false}>
        <TextArea />
      </Form.Item></Col>
    }
  ...
  return (
    <div className="detail-oa">
      ...
      <Drawer visible={props.detailVisible} placement="top" closable={false} destroyOnClose={true} getContainer={false} style={{ position: 'absolute' }}>
        <p className="title">编辑</p>
        <div className="detail-content">
          <div className="sec-title">
            <span className="title-icon"></span>著录项
            <div className="control-btns">
              <Button type="primary" onClick={onSave} style={{ marginRight: 20 }}>保存</Button>
