...
const SelectDailogVisiable = observer((props) => {
  ...
  const { visible = true, params, callback, onChange, extendParams } = props;
  ...
  useEffect(() => {
    debugger;
    YjspSelStore.url = `/api/eps/control/main/${extendParams.spUrl}`;
    ...
    YjspSelStore.setSaveParamsTitle(
      'title',
  ...
  const doOk = (autosubmit) => {
    if (!YjspSelStore.saveParams.id) {
      showMessage('请选择一条申请单或者新建一条申请单', 'warn');
