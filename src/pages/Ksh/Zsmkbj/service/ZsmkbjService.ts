...
class ZsmkbjService extends BaseService implements IEpsService, ITableService {

  findByKey(key: string | number, page: number = 1, size: number = 10, args: object = {}): Promise<any> {
     args['zsmkbjZsmkId']=key;
  ...
  findTree(key: string = '') {
    let params = { 'zsmkbjmc': key }
    return new Promise((resolve, reject) => {
      return super.post({ url: this.url + `/findList`, data:params }).then(res => {
        if (res.status > 300) {
          return reject(res)
        }
        let data = res.data.map(item => { return { id: item.id, title: item.zsmkbjmc, key: item.id } })
