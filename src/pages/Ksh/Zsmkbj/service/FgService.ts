...
class FgService extends BaseService implements IEpsService, ITableService {
  ...
  findTree(key: string = '') {
    ...
    return new Promise((resolve, reject) => {
      return super.post({ url: this.url + `/findList`, data:params }).then(res => {
        ...
        let data = res.data.map(item => { return { id: item.id, title: item.fgmc, key: item.id } })
...
export default new FgService('/api/eps/ksh/fg');
