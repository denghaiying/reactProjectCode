...
class GmapService extends BaseService implements IEpsService, ITableService {
  ...
  findTree(key: string = '') {
    let params = { 'gmapmc': key }
    return new Promise((resolve, reject) => {
      return super.post({ url: this.url + `/findList`, data:params }).then(res => {
        if (res.status > 300) {
          return reject(res)
        }
        let data = res.data.map(item => { return { id: item.id, title: item.gmapmc, key: item.id } })
...
export default new GmapService('/api/eps/ksh/gmap');
