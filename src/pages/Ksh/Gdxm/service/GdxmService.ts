...
class GdxmService extends BaseService implements IEpsService, ITableService {
  ...
  findTree(key: string = '') {
    let params = { 'gdxmmc': key }
    return new Promise((resolve, reject) => {
      return super.post({ url: this.url + `/findList`, data:params }).then(res => {
        if (res.status > 300) {
          return reject(res)
        }
        let data = res.data.map(item => { return { id: item.id, title: item.gdxmmc, key: item.id } })
...
export default new GdxmService('/api/eps/ksh/gdxm');
