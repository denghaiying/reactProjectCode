...
import {FileExclamationOutlined, FileWordOutlined, UploadOutlined} from "@ant-design/icons";
...
const Fzdg = observer((props: IProp) =>{
  ...
  function  show(){
    if( props.record.spzt ==='5'){
      showMessage('定稿状态的志鉴地情资料不能定稿', 'info');
  ...
  return (
    <>
      <Tooltip title="定稿">
        <Button size="small"  style={{fontSize: '12px'}} type={'primary'} shape="circle" icon={<FileWordOutlined /> }
          // onClick={() => {
          //   setIsModalVisible(true);
          // }}
                onClick={show}
      ...
      >
        <Upload
          ...
          data={{id:props.record.id,status:5,whrid:SysStore.getCurrentUser().id,whr:SysStore.getCurrentUser().yhmc}}
          onChange={onChange}
          /* onSuccess={onSuccess}*/
          //    listType="text"
        >
