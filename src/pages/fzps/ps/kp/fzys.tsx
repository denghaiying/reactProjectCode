...
import { FileExclamationOutlined, UploadOutlined} from "@ant-design/icons";
...
const Fzys = observer((props: IProp) =>{
  ...
  function  show(){
    if(props.record.spzt ==='1'){
      setIsModalVisible(true);
    }else{
      showMessage('只有登记状态的志鉴地情资料才能初审', 'info');
    }
  ...
  return (
    <>
      <Tooltip title="初审">
        <Button size="small"  style={{fontSize: '12px'}} type={'primary'} shape="circle" icon={<FileExclamationOutlined />}
                // onClick={() => {
                //   setIsModalVisible(true);
                // }}
          onClick={show}
      ...
      >
        <Upload
          ...
          data={{id:props.record.id,status:3,whrid:SysStore.getCurrentUser().id,whr:SysStore.getCurrentUser().yhmc}}
          onChange={onChange}
          /* onSuccess={onSuccess}*/
          //    listType="text"
        >
