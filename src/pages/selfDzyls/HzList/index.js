...
const List = observer((props) => {
  const store = useLocalStore(() => ({
    ...
    async findaddress(name) {
      debugger
      const res = await ListService.findByKey('', 1, 10, { channelname: name,channeltypelx:'selfstreaming' });
      this.lists = res.list;
    },
  }));
  ...
  useEffect(() => {
    debugger
    store.titlename=props.location.query.name;
    store.findaddress(props.location.query.name);
  }, []);

  return (
    <div className={`${style['list-content']}`}>
      <Header name={store.titlename}/>
