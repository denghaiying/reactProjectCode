...
const Home = observer((props) => {
  const store = useLocalStore(() => ({
    ...
    async findaddress() {
      const response = await fetch.post(
        `/api/streamingapi/content/findaddress`,
      );
      ...
      axios
        ...
        .then((result) => {
          let type = result.data.data?.forecast[0].type;
          let high = result.data.data?.forecast[0].high;
          let low = result.data.data?.forecast[0].low;
  ...
  const jumphz = (type) => {
    history.push(`/runRfunc/hzlist?name=${type}`);
  };

  const yjxx = () => {
    history.push(`/runRfunc/yjxx`);
  };

  const jumpdl = () => {
    cookie.remove('ssotoken');
    history.push(`/`);
  };
    ...
    <div className={`${style['home-content']}`}>
      ...
      <div className={`${style['body']}`}>
        <div className={`${style['common']}`}>
          <div className={`${style['left']}`}>
            <div className={`${style['half-h']}`}>
              ...
              <a href="javascript:;" onClick={() => jumpdl()}>
            ...
            <a href="javascript:;" onClick={() => jump('yj')}>
        ...
        <div className={`${style['common']} ${style['mt-20']}`}>
          <a href="javascript:;" onClick={() => yjxx()}>
            <div className={`${style['left']}`}>
          ...
          <div className={`${style['right']}`}>
            <div className={`${style['half-w']}`}>
            <a href="javascript:;" onClick={() => jumphz('汉中人文')}>
              ...
              <a href="javascript:;" onClick={() => jumphz('汉中历史')}>
