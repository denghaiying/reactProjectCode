...
const List = observer((props) => {
  const store = useLocalStore(() => ({
    ...
    async findaddress() {
      const res = await ListService.findByKey('', 1, 10, { channelname:  '单位介绍',channeltypelx:'selfstreaming'});
      this.lists = res.list;
    },
  }));
  ...
  const menusclick = async (item) => {
    ...
    const res = await ListService.findByKey('', 1, 10, { channelname: xxzt,channeltypelx:'selfstreaming' });
