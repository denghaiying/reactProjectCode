...
class ChannelTypeService extends BaseService implements IEpsStore,ITreeService,ITableService {
  ...
  findTree(key: string): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
        return super.get({ url: this.url + '/queryForList' }).then(res => {
          ...
          console.log("typetree=====",res);
          let data = res.data.map(item => { return { id: item.channeltypebh, title: item.channeltypename, key: item.channeltypebh } })
          return resolve(data)
        }).catch(err => {
          return reject(err)
        })
...
export default new ChannelTypeService('/api/streamingapi/new/channeltype');
