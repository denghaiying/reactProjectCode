...
const customForm = () => {
  ...
  return (
    <>
      ...
      <Form.Item label="属性编号:" name="xysjlxbh" required rules={[{ required: true, message: '请输入编号' }, { max: 10, message: '编号长度不能大于10个字符', }]}>
        <Input allowClear style={{ width: 300 }} />
      </Form.Item>
      <Form.Item label="属性名称:" name="xysjlxmc" required rules={[{ required: true, message: '请输入名称' }]}>
        <Input allowClear style={{ width: 300 }} />
      </Form.Item>
  ...
  // 创建右侧表格Store实例
  const [tableStore] = useState<EpsTableStore>(new EpsTableStore(xysjlxService));
  ...
  return (
    <>
      <EpsPanel title={title}                    // 组件标题，必填
        ...
        tableService={xysjlxService}             // 右侧表格实现类，必填
        ref={ref}                                // 获取组件实例，选填
        formWidth={500}
