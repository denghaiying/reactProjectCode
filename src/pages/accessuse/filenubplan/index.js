...
import {Search, Tab, Input, Button, Form, DatePicker, Checkbox, Table, Pagination, Icon, Grid,Select} from '@alifd/next';
...
const ArchiveInfo = observer(props => {
    ...
    useEffect(() => {
        ...
        Store.queryForPage().then(() => { Store.findMbkList() });
    }, []);
        ...
        <div className="hall-regist">
            <div className="control">
                ...
                <Form inline style={{marginTop: "15px"}}>
                    <FormItem label="模板">
                    <Select name="mbid" style={{ width: "200px" }}>
                        <Select.Option value=" " key="k0" rereadOnly={true}> </Select.Option>
                        {Store.mblist.map(item => <Select.Option value={item.id} key={item.id}>{item.mc}</Select.Option>)}
                    </Select>
                    </FormItem>
                    <FormItem label=" ">
