    ...
    switch (step) {
      case 0:
        ...
        if (LydjStore.editRecord && LydjStore.editRecord.id) {
          LydjStore.opt === 'view' && setStep(step + 1);
      ...
      case 3:
        if (LydjStore.opt !== 'view') {
          fieldRate.validate((errors, values) => {
            if (!errors) {
              LydjStore.saveDjData({ ...values, status: 'C' });
      ...
      <Row>
        ...
        <Col span={19}>
          <Row>
            ...
            <Col>
              <FormItem fullWidth label=" " colon={false} {...formItemLayout}>
                <Radio.Group name="jyrxz">
    ...
    <Fragment>
      <div className="dakcx">
        <TreeSelect
          ...
          style={{ width: '20%' }}
          readOnly={LydjStore.opt === 'view'}
          disabled={LydjStore.opt === 'view'}
          notFoundContent="未找到数据"
          {...fieldDak.init('dakid', {
            rules: [
              {
                required: true,
                message: `${formatMessage({ id: 'e9.info.data.require' })}`,
              },
            ],
          })}
        />
        ...
        <Col span={16}>
          ...
          >
            {LydjStore.tpageno.map((item, index) => (
              <Tab.Item key={index} title={`查询结果${index + 1}`} closeable>
                <Table.StickyLock
                  isZebra
                  bordered="true"
                  fixedHeader
          ...
          <Tab>
            <Tab.Item title="利用明细">
              <Table.StickyLock
                ...
                bordered="true"
                maxBodyHeight={maxBodyHeight}
                dataSource={LydjStore.lymxData.results || []}
              ...
              >
                <Table.Column
                  ...
                  width="80px"
                  alignHeader="center"
                  cell={(v, index) => index + 1}
                />
                <Table.Column
                  title="操作"
                  width="100px"
                  alignHeader="center"
                  cell={renderLymxTableCell}
                />
                <Table.Column
                  title={<>实体借阅</>}
                  width="100px"
                  ...
                  alignHeader="center"
                  align="center"
                  cell={(v, rowIndex, record) => (
                    <IceCheckBox
                      checked={v === 'Y'}
                      onChange={(checked) => {
                        LydjStore.checkLymxRecord(rowIndex, checked);
                      }}
                    />
                  )}
                />
  ...
  const renderLydj = () => (
    ...
    >
      ...
      <Row>
        ...
        <Col span={6}>
          <FormItem fullWidth required label="利用效果：" {...formItemLayout}>
            <IceSelect
              name="lyxg"
              placeholder="请选择利用效果"
              readOnly={LydjStore.opt === 'view'}
      ...
      <div className="editform">
        ...
        <div className="editContext">
          ...
          <div className="effooter">
            {/* <UserSwitchOutlined /> */}
            {step === 0 && LydjStore.opt !== 'view' && (
              <IceButton type="normal" onClick={doGenUser}>
                <IceIcon type="account" />
                生成临时账户
              </IceButton>
            )}
            {step === 1 && LydjStore.opt !== 'view' && (
              <>
                <IceButton.Group>
                  <IceButton type="normal" onClick={doTranferAppro}>
                    转审批
                  </IceButton>
                </IceButton.Group>
                <IceButton.Group style={{ marginLeft: 20 }}>
                  <Tooltip title="实体出库登记">
                    <IceButton type="normal" onClick={doDiaoJuan}>
                      调卷
                    </IceButton>
                  </Tooltip>
                  {LydjStore.paramValue['DADTS002'] !== 'Y' && (
                    <IceButton IceButton type="normal" onClick={doStlq}>
                      出库并领取
                    </IceButton>
                  )}
                  {LydjStore.paramValue['DADTS003'] !== 'Y' && (
                    <IceButton type="normal" onClick={doStgh}>
                      归还并入库
                    </IceButton>
                  )}
                </IceButton.Group>
        ...
        <Modal
          ...
          bodyStyle={{ paddingTop: '2px', height: '70vh' }}
          maskClosable={false}
          width={'80%'}
          footer={[
          ...
          <Spin spinning={LydjStore.saving} size="large">
            <Row align="stretch" className="lydj-qx">
                ...
                <Typography>
                  ...
                  <Typography.Paragraph>
                    <Checkbox
                      ...
                      onChange={(e) => LydjStore.chaKaiFangArc(e.target.checked)}
                    >
                      仅仅查看开放档案
                    </Checkbox>
                  </Typography.Paragraph>
              ...
              >
                <Row>
                  <Col span={18}>
                    {LydjStore.qxCurName && LydjStore.qxCurName === 'attr_nd'
                  ...
                  <Col>
                    <Button.Group>
                      <Button type="primary" onClick={() => {
                        LydjStore.qxCurName && LydjStore.setAllQxData(LydjStore.qxCurName, true);
                      }}>全选</Button>
                      <Button type="primary" onClick={() => {
                        LydjStore.qxCurName && LydjStore.setAllQxData(LydjStore.qxCurName, false);
                      }}>全清</Button>
                    </Button.Group>
                  </Col>
                </Row>
                <Spin size="large" spinning={LydjStore.qxMapLoading}>
                  <Space wrap style={{ overflowY: 'auto', maxHeight: '70vh' }}>
                    {LydjStore.qxCurName &&
                      ...
                      LydjStore.qxMap[LydjStore.qxCurName]
                        .filter(
                          (o) => LydjStore.qxCurName !== 'attr_nd' &&
                            (!LydjStore.qxFilterStr ||
                              o.includes(LydjStore.qxFilterStr)) || (LydjStore.qxCurName === 'attr_nd' && (
                                !LydjStore.qxNdFilterStr || LydjStore.qxNdFilterStr.length != 2 || !LydjStore.qxNdFilterStr[0]
                                || (o >= LydjStore.qxNdFilterStr[0] && o <= LydjStore.qxNdFilterStr[1])
                              ))
                        )
                        .map((o) => (
                          <CheckableTag
