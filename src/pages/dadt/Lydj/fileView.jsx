...
const FileView = observer(() => {
    ...
    if (numPages > 0) {
      ...
      if (dom) {
        const data = await Promise.all(
          util.num2arr(numPages).map((n) => docproxy.getPage(n + 1)),
        );
        const vws = data.map((page) => page.getViewport({ scale: 1 }));
        const mw = Math.max.apply(
          Math,
          vws.map((vw) => vw.width),
        );
      ...
      if (dom.attachEvent) {
        dom.attachEvent('onload', () => {
          ...
          if (LydjStore.opt !== 'view') {
            LydjStore.setRecordValue(
              'fyys',
              parseInt(LydjStore.editRecord.fyys || 0) +
                parseInt((selectPages && selectPages.length) || nums),
            );
          }
        });
      } else {
  ...
  const doPrintLyPdf = () => {
    ...
    const dom = printRef.current;
    const blob = new Blob([LydjStore.files[selectedKey].data], {
        ...
        <Row>
          ...
          <Col>
            <input
              ...
              max="400"
              value={scale * 100}
              onChange={(e) => setScale((e.target.value / 100).toFixed(2))}
            />
            ...
            {LydjStore.fileshowtype === 'D' && (
              ...
              >
                <SvgIcon type="iconprint" style={{ marginRight: 2 }} />
                {`打印${
                  (selectPages &&
                    selectPages.length > 0 &&
                    '第'.concat(selectPages.join('、'), '页')) ||
                  '所有页'
                }`}
              </Button>
            )}
    ...
    >
      <Row align="stretch" className="docViewDailogContent">
        <Col span={4} style={{ borderRight: '1px dashed #c4c6cf' }}>
          <Nav type="normal" embeddable selectedKeys={[selectedKey]}>
            {LydjStore.fileData &&
              LydjStore.fileData.map((o) => (
                <Nav.Item
                  key={o.fileid}
                  onClick={() => {
                    viewFile(o.grpid, o.fileid);
        ...
        <Col span={20}>
          <div className="docViewDailogContentInner" ref={ref}>
            <Loading
              visible={LydjStore.downloadingfile || LydjStore.downloadingpdf}
            />
            {!LydjStore.downloadingfile && !LydjStore.downloadingpdf && (
              <div style={{ width: '100%', height: '100%' }}>
                {selectedKey && (
                  <Document
                    ...
                    file={
                      LydjStore.files[selectedKey] &&
                      LydjStore.files[selectedKey].data
                    }
                    onLoadSuccess={(docproxy) => {
                      onDocumentLoadSuccess(docproxy);
                    }}
                  >
