...
const FullSearch9 = observer(props => {
  ...
  const showXcdApply = () => {
    ...
    if (ParamsManageStore.xcdStatus && ParamsManageStore.xcdStatus.message == "Y") {
      return <Button onClick={onOpen} type="primary" size="large"> 协查申请</Button>
    }
  };
  ...
  const getTimeAndtotal = () => {
    if (data.length <= 0 || data.code == 500) {
      return (
        <p className="total" style={{ marginLeft:'30px'}}>检索耗时 0 秒，为您找到相关结果约 0 条</p>)
    } else {
      return (
        <p className="total" style={{ marginLeft:'30px'}}>检索耗时{data.responseTime / 1000000000}秒，为您找到相关结果约 {data.total} 条</p>)
    }
  }
        ...
        <div className="full-search">
          <div className="control">
            <FormItem name="searchValue">
              <Search type="secondary"  style={{height:'33px'}}  size='medium'  searchText={<span>搜索</span>} onChange={Store.onChange} onSearch={doSearchAction} />
            </FormItem>
          </div>
        ...
        <div style={{ height: 450 }}>
          <Form form={xcdForm} {...formItemLayout}>
            <Row >
              ...
              <Divider orientation="left" plain>协查信息</Divider>
              <Col span={span}>
                {/* <div style={style}> */}
                <FormItem label="申请人员:" name="yhmc" initialValue={SysStore.getCurrentUser().yhmc} labelCol={{ span: 6 }}>
                  <Input
                    hasClear
                    placeholder="请输入申请人"
                    disabled />
              ...
              <Col span={span}>
                {/* <div style={style}> */}
                <FormItem label="申请单位:" name="dwmc" initialValue={SysStore.getCurrentCmp().mc}>
              ...
              <Col span={span} >
                {/* <div style={style}> */}
                <FormItem label="申请部门:" name="bm" initialValue={SysStore.getCurrentUser().orgmc}>
                  <Input
                    hasClear
                    placeholder="请输入申请部门"
                    disabled />
              ...
              <Col span={span} >
                {/* <div style={style}> */}
                <FormItem label="手机号码:" name="sj" rules={[{ pattern: /^1[3456789]\d{9}$/, message: '请输入有效的手机号!' },]} labelCol={{ span: 6 }} initialValue={SysStore.getCurrentUser().sjh}>
                  <Input
                    hasClear
                    placeholder="请输入手机号码："
                  />
              ...
              <Col span={8} >
                {/* <div style={style}> */}
                <FormItem label="电子邮箱:" name="mail" rules={[{ pattern: /^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/, message: '请输入有效的邮箱!' }]}  initialValue={SysStore.getCurrentUser().mail}>
                  <Input
                    hasClear
                    placeholder="请输入电子邮箱"
                  />
              ...
              <Divider orientation="left" plain>协查操作</Divider>
              <Col span={span}>
                {/* <div style={style}> */}
                <FormItem label="所属全宗:" name="ssjzid" style={{ height: 40 }} labelCol={{ span: 6 }} >
              ...
              <Col span={span} >
                <FormItem label="利用目的:" name="lymd" rules={[{ required: true, message: '请选择利用目的' }]}>
                  <Select
                    style={{ width: 210 }}
                    placeholder="请选择利用目的"
              ...
              <Col span={span} >
                {/* <div style={style}> */}
                <FormItem label="借阅天数:" name="jyts" initialValue={Store.pDALYF002} rules={[{ required: true, message: '借阅天数不允许为空!' }]}>
              ...
              <Col span={24} style={{ height: 98 }}>
                <FormItem label="借阅说明:" name="bz" labelCol={{ span: 2 }} rules={[{ required: true, message: '请输入借阅说明' }]} >
                  <Input.TextArea placeholder="请输入借阅说明："
                    showCount
                    rows={3}
                    maxLength={500}
                    style={{ width: 870 }} />
