...
import File from '@/pages/Tyjk/zjksj/File';
...
const all = observer(props => {
  ...
  const  showDakFile  = async (record) => {
    debugger;
    if (record.attr_fileid && record.attr_fjs && record.attr_fjs > 0) {
      debugger;
      const tmresponse = await fetch.get(
        `/api/eps/control/main/daly/queryForId?id=${record.attr_tmid}&bmc=${record.attr_bmc}`,
      );
      debugger;
      LydjStore.showFiles('D', {
        bmc: record.attr_bmc,
        tmid: record.attr_tmid,
        dakid: record.attr_dakid,
        grpid: tmresponse.data.filegrpid,
        // filegrpid:tmresponse.data.filegrpid,
  ...
  const  addDakFileToLymx  = async (record) => {
    await LydjStore.addTmToLymx('',record)
      ...
      return (
        <>
          <span className="icon-part"
           onClick={() => addDakFileToLymx(value)}
           >
             <span className="iconfont icon-book"></span>加入查档</span>
        </>
      )
    }
  }
        ...
        data && data.data && data.data.map && data.data.map(item => (
          <li className="item" key={item.source.id}>
              ...
              item.source.attr_sylx === 'yw' ?
                <div className="file-place">
                  ...
                  <div className="pdf_right">
                    {/* 原文在这里 */}
                    <span
                    //  onClick={() => ywClick(item.source.attr_tmid, item.source.attr_dakid, item.source.attr_fileid, item.source.attr_wjtm, item.source.attr_dwid, item.source.attr_wjlx, item.source.attr_bmc, item.source.attr_sylx)}
                        onClick={() =>showDakFile(item.source)}
                    >
            ...
            <div className="bottom">
              ...
              <span className="icon-part" onClick={() => Store.addLobby(item.source.attr_tmid, item.source.attr_dakid, item.source.id, item.source.attr_bmc)}><span className="iconfont icon-book"></span>加入查档</span> */}
              </div>
            </div>
          </li>
        ))
        ...
        data && data.data && data.data.map && data.data.map(item => (
          <li className="item" key={item.source.id}>
            ...
            <div className="bottom">
              ...
              <span className="icon-part"><span className="iconfont icon-book"></span>加入查档</span> */}
          ...
          <li className="item" key={item.source.id}>
            ...
            <div className="file-place">
              ...
              <div className="pdf_right">
                ...
                <span onClick={() =>showDakFile(item.source)}>
                  <p className="title-a"><div dangerouslySetInnerHTML={{ __html: item.source.attr_wjbt }} /></p>
                </span>
                <p className="summary"><div dangerouslySetInnerHTML={{ __html: item.source.text }} /></p>
              </div>
