...
const Cart1 = observer((props) => {
  ...
  const [tableStore, setTableStore]= useState<EpsTableStore>(new EpsTableStore(CartService));
  ...
  const CartStore = useLocalObservable(() => (
    {
      ...
      async query() {
        const url = "/api/eps/control/main/daly/queryForCart?yhid="+SysStore.getCurrentUser().id ;
      ...
      async getUserOptionByDALYF002() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DALYF002&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
      ...
      async getUserOptionByJysm() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DALYS002&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
      ...
      async getUserOptionByLymd() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DAJYC002&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
        console.log('responen002',response);
        if (response.status === 200) {
          if (response.status === 200) {
      ...
      async getLx() {
        ...
        if (response && response.status === 200) {
          ...
          //  this.lxdata=sjData;
          if(response.data) {
            this.lxdata = response.data.map(o => ({'id': o.bh, 'label': o.mc, 'value': o.bh}));
          }
        } else {
          return
        }
      ...
      async getLymd() {
        ...
        if (response && response.status === 200) {
          ...
          this.lymdData = response.data.map(o => ({ 'id': o.bh, 'label': o.mc, 'value': o.bh }));
        } else {
          return
        }
      },
      ...
      async getDwInfo() {
        const url="/api/eps/control/main/dw/queryForId?id="+SysStore.getCurrentUser().dwid;
        const response=await fetch.get(url);
      ...
      async getUserOptionByTxtxt() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DALYS012&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
      ...
      async getUserOptionByDyXz() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DALYS004&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
      ...
      async getUserOptionByFksshow() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DALYS011&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
      ...
      async getUserOptionBySywzShow() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DALYS014&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
      ...
      async getUserOptionByTx() {
        const url="/api/eps/control/main/params/getParamsDevOption?code=DALYS017&yhid="+SysStore.getCurrentUser().id;
        const response=await fetch.get(url);
      ...
      async apply(bdInfo,data) {
        ...
        // 获取默认的查看、打印、下载方式
        const urlck="/api/eps/control/main/params/getParamsDevOption?code=DALYS030&yhid="+SysStore.getCurrentUser().id;
        ...
        const urlJyts="/api/eps/control/main/params/getParamsDevOption?code=DALYF003&yhid="+SysStore.getCurrentUser().id;
        const isJytsRes= await fetch.get(urlJyts);
        const isJyts=isJytsRes.data;

        // var jytsData = getDefaultv("DALYF002"); //协查单或借阅单的借阅天数，默认值为1
        const urljytsData="/api/eps/control/main/params/getParamsDevOption?code=DALYF002&yhid="+SysStore.getCurrentUser().id;
        ...
        if (jytsData && isJyts !== "N" && (bdInfo.jyts > jytsData || bdInfo.jyts < 2)) {
          ...
          //     console.log(error);
          //   });
          const response = await new HttpRequest('').post({url: `/eps/control/main/daly/apply`, params: values} );
  ...
  const onFinish = (values: any) => {
    form.validateFields().then(data => {
      ...
      CartStore.apply(data,gridData).then(res => {
      ...
      <Space direction="vertical">
        ...
        >
          <Row>
            <Col >
              <div className="btns">
                ...
                <EpsModalButton  name="附件" title="附件" width={1200}   useIframe={true}
                                 params={{ docTbl:"ATTACHDOC",docGrpTbl:"DOCGROUP",grpid:filegrpid,idvs:idv,
                                   wrkTbl:"JYD",lx:null,atdw:"DEFAULT",tybz:"N",
                                   whr:yhmc,whsj:getDate,fjsctrue:true}}
          ...
          <Collapse   activeKey={1} >
            ...
            >
              {/*<Form labelCol={{span: 6, offset: 3}}>*/}
              <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} style={{width: '96%'}}>
          ...
          <Collapse key={2} >
            <Panel header="借阅操作" key="2" forceRender={true}>
              {/*<Form labelCol={{span: 6, offset: 3}}>*/}
              <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} style={{ height:  height,width: '96%'}}>
                <Col span={span} >
                  <Form.Item
                    label="利用目的"
                    name="lymd"
                    rules={[{ required:checkLymd, message: '利用目的不允许为空!' }]}
                  >
                ...
                <Col span={span}>
                  ...
                  >
                    <InputNumber
                      ...
                      defaultValue={jyts} style={{width : 198}}  controls={true} />
                  </Form.Item>
                </Col>
                <Col span={span}>

                  <Form.Item label="操作" name="action">
                    <Checkbox.Group key={defaultVals}  options={plainOptions} defaultValue={defaultVals}
                                    onChange={onChange}  />

                  </Form.Item>
                </Col>
              </Row>
              <Divider orientation="left" plain>利用说明</Divider>
              <Form.Item labelCol={{span: 24}}   name="jysm" rules={[{ required:checkJysm, message: '利用说明不允许为空!' }]}>
                <Input.TextArea placeholder="请输入对利用目的详细说明" rows={4}  style={{width: '96%'}} ></Input.TextArea>
              </Form.Item>
          ...
          <Collapse activeKey={key} onChange={(val) => setKey(val)}>
            <Panel header="借阅文件" key="3" forceRender={true}>
              <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}  style={{height:  height,width: '96%'}}>
                ...
                <Col span={span}>
                  <Form.Item
                    hidden
                    name="yhid" initialValue={SysStore.getCurrentUser().id}>
                    <Input hidden />
                  </Form.Item>
                </Col>
                ...
                <Col span={span}>
                  <Form.Item
                    name="dwid" hidden initialValue={SysStore.getCurrentUser().dwid}>
                    <Input hidden />
                  </Form.Item>
                </Col>
                ...
                <Col span={span}>
                  <Form.Item
                    name="bmid" hidden initialValue={SysStore.getCurrentUser().bmid}>
                    <Input disabled />
                  </Form.Item>
                </Col>
              </Row>
            ...
            <Panel header="借阅清单" key="4"  collapsible="disabled" forceRender={true}>
              <div
                style={{height: '320px'}}
              >
                <Table columns={source} dataSource={CartStore.cartDataSource} bordered scroll={{ x: 'max-content' }}
                       pagination={false}
                       className="my-table"
                       expandable={{
