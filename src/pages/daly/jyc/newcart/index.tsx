...
const NewCart = observer((props) => {
  ...
  const [dacdfsvisivle, setDacdfsvisivle] = useState(true);
  const onChange = (list) => {
    console.log('list', list);
    //setCheckedData(list);
  };
  const dacdfsChange = (values: any) => {
    if (values.dacdfs == '1') {
      setDacdfsvisivle(true);
      form.setFieldsValue({ yjdz: '' });
    } else {
      setDacdfsvisivle(false);
      form.setFieldsValue({
        yjdz:
          '姓名：' + yhmc + '      手机号码：' + SysStore.getCurrentUser().sjh,
      });
    }
    //setCheckedData(list);
  };
  ...
  const onotherChange = (e) => {
    ...
    if (e.target.name == 'zzclfyj') {
      if (e.target.checked) {
        setPlainOptions(['查看']);
        form.setFieldsValue({ action: ['查看'] });
      } else {
        setPlainOptions(['查看', '打印', '下载']);
      }
    }
  ...
  const dacdfsdata = [
    { id: '1', label: '现场领取', value: '1' },
    { id: '2', label: '邮寄', value: '2' },
  ];
  const source = [
    {
      title: '序号',
        ...
        >
          ...
          >
            <Col span={span}>
              ...
              >
                <Input placeholder="举例：关于xx事项（项目）调阅档案申请" />
              </Form.Item>
            </Col>
            ...
            <Col span={span}>
              <Form.Item
                label="归还日期"
                rules={[{ required: true, message: '预计归还日期不允许为空!' }]}
                name="ghrq"
              >
                <DatePicker
            ...
            <Col span={span}>
              <Form.Item
                label="传递方式"
                name="dacdfs"
                rules={[{ required: true, message: '传递方式不允许为空!' }]}
              >
                <Select
                  onChange={(value) => dacdfsChange({ dacdfs: value })}
                  options={dacdfsdata}
                />
              </Form.Item>
            </Col>
            <Col span={12} style={{ height: 60 }}>
              <Form.Item label="邮寄地址" labelCol={{ span: 2 }} name="yjdz">
                <Input.TextArea
                  disabled={dacdfsvisivle}
                  placeholder="请输入对利用目的详细说明"
                  rows={2}
                ></Input.TextArea>
              </Form.Item>
