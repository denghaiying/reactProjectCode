...
const kkjs = (props) => {
  ...
  const ontaleSelect = async (record) => {
    ...
    const params: ArchParams = {
      umid: jdumid,
      umname: `${record.dakmc}【${jdanme}】`,
      bmc: dak.data.mbc,
      dakid: dakid,
      lx: dak.data.lx,
      wzlk: 'N',
      type: 'K',
      path: `/runRfunc/archManage/${dakid}/${record.dakzt}`,
      mc: dak.data.mc,
    ...
    runFunc(params);
    // history.push({ pathname: `/eps/dagl/archManage/${dakid}/${record.dakzt}`, query: params });
  };
  ...
  const doSearchAction = (formData, tableStore) => {
    ...
    var datmzt = '';
    if(formData.tmzt === undefined || formData.tmzt === '') {
        // message.warning({type: 'warning', content: '请选择选择档案库状态'});
        // return;
      datmzt=',4';
    } else {
  ...
  const customAction = (store, rows) => {
    return [
      <>
        ...
        >
          <Form.Item name="tmzt" initialValue={'4'}>
            <Select
              mode="multiple"
              maxTagCount={3}
              style={{ width: 300 }}
      ...
      {() => (
        <div className="oa-manage">
          ...
          <div className={expand ? 'content' : 'content hideExpand'}>
            <div className="tree">
              <DakTree store={Store} treeData={Store.treeData} />
            </div>
            <div className="right">
              <EpsPanel
                title="跨库检索"
