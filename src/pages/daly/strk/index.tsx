...
function stlq() {
  ...
  const ref = useRef();
  useEffect(() => {
    NewDajyStore.queryStrkZt();
    OptrightStore.getFuncRight('DALY058');
  }, []);
  ...
  const source: EpsSource[] = [
    ...
    },
    {
      ...
      formType: EpsFormType.Input,
      render: () => {
        return NewDajyStore.selectlx === '3' ? '待入库' : '已入库';
      },
    },
    {
      title: '借阅单号',
  ...
  const OnSearch = (values: any, store: EpsTableStore) => {
    NewDajyStore.selectlx = values.lx;
  ...
  const onPut_Examine = async (store, ids) => {
    if (ids.length >= 1) {
      NewDajyStore.stlqghspzt = '9';
      NewDajyStore.stlqghop = '实体入库';
      NewDajyStore.stlqghdazt = '已入库';
      for (var i = 0; i < ids.length; i++) {
        await NewDajyStore.updateStrk(ids[i]);
    ...
    return [
      <>
        <Form
          ...
          initialValues={{ lx: '3', yhid: yhid, gnid: gnid }}
        >
          <Form.Item
            label=""
            className="form-item"
        ...
        &nbsp;&nbsp;
        {OptrightStore.hasRight('DALY058', 'SYS101')&&
        <Button
          className="form-item"
          type="primary"
          disabled={NewDajyStore.selectlx === '3' ? false : true}
        ...
        <Modal
          title={<span className="m-title">请输入库说明:</span>}
