...
function stlq() {
  ...
  const ref = useRef();
  useEffect(() => {
    NewDajyStore.queryStlqZt();
    OptrightStore.getFuncRight('DALY053');
  }, []);
  ...
  const source: EpsSource[] = [
    ...
    },
    {
      ...
      formType: EpsFormType.Input,
      render: () => {
        return NewDajyStore.selectlqlx === '1' ? '待领取' : '已领取/待归还';
      },
    },
    {
      title: '借阅单号',
  ...
  const OnSearch = (values: any, store: EpsTableStore) => {
    NewDajyStore.selectlqlx = values.lx;
    store && store.findByKey(store.key, 1, store.size, values);
  };

  const check_onPut_Lq = async (rec, store) => {
    if (rec && rec.length >= 1) {
  ...
  const onPut_Examine = async (store, records) => {
    if (records && records.length >= 1) {
      ...
      await NewDajyStore.updateStlq(records, values.note);
  ...
  // 自定义查询按钮
  const customAction = (store: EpsTableStore, record: any[]) => {
    return [
      <>
        <Form
          layout="inline"
          ...
          initialValues={{ lx: '1', yhid: yhid, gnid: gnid }}
        >
        ...
        &nbsp;&nbsp;
        {OptrightStore.hasRight('DALY053', 'SYS101')&&
        <Button
          className="form-item"
          type="primary"
          disabled={NewDajyStore.selectlqlx === '1' ? false : true}
          onClick={() => check_onPut_Lq(record, store)}
        >
          领取
        </Button>}
        {/* <Button
          className="form-item"
          type="primary"
          onClick={() => check_onPut_TmLq(ids, store)}
        >
          条码领取
        </Button>
        <Button
          className="form-item"
          type="primary"
          onClick={() => check_onPut_DzBqTmLq(ids, store)}
        >
          电子标签条目领取
        </Button>
        <Button
          className="form-item"
          type="primary"
          onClick={() => check_onPut_DzBqHzLq(ids, store)}
        >
          电子标签盒子领取
        </Button> */}
        <Modal
          title={<span className="m-title">请输入领取说明:</span>}
          visible={examine_visible}

          onOk={() => {
            onPut_Examine(store, record), ref.current?.clearTableRowClick();
          }}
          onCancel={() => setExamineVisible(false)}
        >
          <Form className="schedule-form" form={fields}>
