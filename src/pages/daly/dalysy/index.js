...
const Dalysy = observer((props) => {
      ...
      if (response.status === 200) {
        ...
        if (response.data?.length > 0) {
          for (var i = 0; i < response.data?.length; i++) {
            ...
            if (ysj.tph === 'img01') {
              newKey.img = hyda;
            }else if (ysj.tph === 'img02') {
              newKey.img = dszn;
            }else if (ysj.tph === 'img04') {
              newKey.img = zqda;
            }else{
              newKey.img = daly;
            }
  ...
  const openclickfunc = (e) => {
    ...
    if(e==='DADT0002'){
      //params.path=`/runRfunc/dalydjn`;
      params.path=`/runRfunc/cddj`;
      params.umname=`利用登记`;
    }
    if(e==='DALY001'){
      params.path=`/runRfunc/daly`;
      params.umname=`档案利用`;
    }
    if(e==='DATJ050'){
      params.path=`/runRfunc/dalydjntj`;
      params.umname=`利用情况统计`;
    }
    runFunc(params);
  };
