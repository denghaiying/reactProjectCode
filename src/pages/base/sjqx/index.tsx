...
const Sjqx = observer((props) => {
  ...
  const sjqxStore = useLocalObservable(() => ({
    ...
    async queryQwtmgzList() {
      const response = await fetch.get(
        `/api/eps/control/main/sjqxgz/queryForList?lx=A`,
      );
      if (response && response.status === 200) {
        this.qwtmgzlstA = response.data.map((o) => ({
          id: o.id,
          label: o.code + '|' + o.name,
          value: o.id,
        }));
    ...
    async queryQwywgzList() {
      const response = await fetch.get(
        `/api/eps/control/main/sjqxgz/queryForList?lx=B`,
      );
      if (response && response.status === 200) {
        this.qwywgzlstB = response.data.map((o) => ({
          id: o.id,
          label: o.code + '|' + o.name,
          value: o.id,
        }));
    ...
    async queryDakRole() {
      ...
      const response = await fetch.get(
        `/api/eps/control/main/dakrole/queryForDakroleList`,
      );
      if (response.status === 200) {
        if (response.data.length > 0) {
  ...
  useEffect(() => {
    ...
    if (props.dwid != undefined) {
      sjqxStore.getRolerenderlist(props.dwid);
      setInitParams(props);
      // treeStore.key=props.dwid;
    } else {
      sjqxStore.getRolerenderlist(null);
    }
  }, []);
