...
class Sjzdmxervice extends BaseService implements IEpsService, ITableService {
    ...
    findByKey(key: string | number, page: number = 1, size: number = 10, args: object = {}): Promise<any> {
        ...
        if (key) {
            ...
            return new Promise((resolve, reject) => {
                return super.get({ url: this.url + '/queryTree', params: args }).then(res => {
                    if (res.status > 300) {
                        console.log("res", res)
                        return reject(res)
                    }
    ...
    findSelectByKey(key: string | number, page: number = 1, size: number = 10, args: object = {}): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            ...
            return super.get({ url: this.url + '/querySjzdList', params: args }).then(res => {
                if (res.status > 300) {
                    return reject(res)
                }
                return resolve(res.data)
...
export default new Sjzdmxervice('/api/eps/control/main/sjzdmx');
