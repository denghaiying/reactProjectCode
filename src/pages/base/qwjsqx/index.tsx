...
import SelectService from '@/pages/base/qwjsqx/selectService';
import QwjsqxService from '@/pages/base/qwjsqx/qwjsqxService';
import DwService from './DwService';
import EpsTreeStore from '@/eps/components/panel/EpsPanel2/EpsTreeStore';

//const FormItem = Form.Item;

const Qwjsqx = observer((props) => {
  /**
   * 获取当前用户
   */
  ...
  const [tableStore, setTableStore] = useState<EpsTableStore>(
    new EpsTableStore(QwjsqxService),
  );
  const [treeStore, setTreeStore] = useState<EpsTreeStore>();

  // const { TextArea } = Input;

  const umid = 'WORKFLOW0002';
  /**
   * childStore
   */
  ...
  const editlxChange = (value) => {
    if (value == '2') {
      ...
      if (tableStore) {
        ...
        fetch
          .get(
            `/api/eps/control/main/dak/queryForList?orderby=DAK_MBC&dw=` + dwid,
          )
          .then((res) => {
  ...
  const ref = useRef();
  const [qjjslist, setQjjslist] = useState<
    Array<{ id: string; label: string; value: string }>
  >([]);
  // const [roleFirst,setRoleFirst]=useState<string>('');

  //const [treeStoreId, setTreeStoreId]= useState<EpsTreeStore.treeList ? EpsTreeStore.treeList[0]:''>();
  const [treeStoreId, setTreeStoreId] = useState('');
  ...
  useEffect(() => {
    ...
    console.log('dsfsdfdff======', ref.current.getTreeStore());
    // setTreeStoreId(ref.current.getTreeStore().treeList?ref.current.getTreeStore().treeList[0]:SysStore.getCurrentUser().dwid);
    // treeStore.key = treeStore.treeList ?  treeStore.treeList[0] : ''
  }, []);
  ...
  useEffect(() => {
    const settarDs = () => {
      setSjgzlst(qwjsqxStore.qwgzlst);
    };
