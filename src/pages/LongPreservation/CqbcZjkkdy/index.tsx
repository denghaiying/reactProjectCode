...
import kkdyService from './service/CqbcZjKkdyService';
import { Form, Input, message, Select} from 'antd';
import { observer } from 'mobx-react';
import { EpsSource, ITable } from '@/eps/commons/declare';
import zjjkpzService from '../CqbcZjjkpz/service/CqbcZjJkpzService';
...
const cqbcZjkkdy = observer((props) => {
  ...
  const jkonChange =  async (value) => {
    let url="/api/eps/lg/zjkjkpz/findById?id="+value;
      const response =await fetch.post(url);
       if (response.status === 200) {
        setjklx(response.data.type);
      }
};
  ...
  return (
    <>
        <EpsPanel
          ...
          treeService={zjjkpzService}                  // 左侧树 实现类，必填
