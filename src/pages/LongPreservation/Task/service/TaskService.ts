...
class TaskService extends BaseService implements IEpsService, ITableService {
  ...
  rule(record) {
    return new Promise((resolve, reject) => {
      return super.post({ url: this.url+"/rule", data: record}).then(res => {
        debugger
       if (res.status === 201) {
          return resolve(res.data);
        }
  ...
  reset(record) {
    return new Promise((resolve, reject) => {
      return super.post({ url: this.url+"/reset", data: record}).then(res => {
       if (res.status === 201) {
          return resolve(res.data);
        }
        return reject(res.data)
...
export default new TaskService('/api/api/task');
