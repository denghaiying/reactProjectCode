...
import storeInfoService from './service/StoreInfoService';
...
const StoreInfo = observer((props) => {
const ref = useRef();
    ...
    <>
     <Form.Item label="名称:" name="name" required rules={[{ required: true, message: '请输入名称' }]}>
        <Input allowClear style={{ width: 300 }} />
          ...
          <Form.Item label="IP:" name="ip" required rules={[{ required: true, message: '请输入IP' }]}>
        <Input allowClear style={{ width: 300 }} />
      </Form.Item>
        <Form.Item label="盘符:" name="driver" required rules={[{ required: true, message: '请输入盘符' }]}>
        <Input allowClear style={{ width: 300 }} />
      </Form.Item>
...
const customAction = (store: EpsTableStore, ids: any[]) => {
  ...
  // 创建右侧表格Store实例
  const [tableStore] = useState<EpsTableStore>(new EpsTableStore(storeInfoService));
  ...
  return (
    <>
      <EpsPanel title={title}                    // 组件标题，必填
        ...
        tableService={storeInfoService}             // 右侧表格实现类，必填
        ref={ref}                                // 获取组件实例，选填
        formWidth={500}
