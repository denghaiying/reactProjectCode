...
import eepFileService from './service/EepFileService';
...
const customAction = (store: EpsTableStore, ids: any[]) => {
  ...
  // 创建右侧表格Store实例
  const [tableStore] = useState<EpsTableStore>(new EpsTableStore(eepFileService));
...
useEffect(() => {
  ...
  tableStores.findByKey("",1,50,{'aipid':props.aipid,'tmid':props.tmid});
}, [props]);
  ...
  return (
    <>
      <EpsPanel title={title}                    // 组件标题，必填
        ...
        initParams={{'aipid':props.aipid,'tmid':props.tmid}}
      >
      </EpsPanel>
    </>
  );
