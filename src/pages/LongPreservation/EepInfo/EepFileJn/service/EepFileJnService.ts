...
class EepFileJnService extends BaseService implements IEpsService, ITableService {

  findByKey(key: string | number, page: number = 1, size: number = 10, args: object = {}): Promise<any> {
      return new Promise((resolve, reject) => {
     return super.get({url: this.url+`/queryFiles/`+args.aipid+'/'+args.tmidJn})
        .then(response => {
...
export default new EepFileJnService('/api/api/eepinfo');
