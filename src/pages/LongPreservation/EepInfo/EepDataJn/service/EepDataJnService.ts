...
class EepDataJnService extends BaseService implements IEpsService, ITableService {

  findByKey(key: string | number, page: number = 1, size: number = 10, args: object = {}): Promise<any> {
    return new Promise((resolve, reject) => {
     return super.get({url: this.url+`/queryDataJn/`+args.aipid})
        .then(response => {
...
export default new EepDataJnService('/api/api/eepinfo');
