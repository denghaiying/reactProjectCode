...
import eepDataJnService from './service/EepDataJnService';
...
const EepDataJn = observer((props) => {
const ref = useRef();
...
const customAction = (store: EpsTableStore, ids: any[]) => {
  ...
  // 创建右侧表格Store实例
  const [tableStore] = useState<EpsTableStore>(new EpsTableStore(eepDataJnService));
...
useEffect(() => {
  const querymbzlxList =  async (value) =>{
          let url="/api/api/eepinfo/queryEepColumnJn";
  ...
  return (
    <>
      <EpsPanel title={title}                    // 组件标题，必填
        ...
        tableService={eepDataJnService}             // 右侧表格实现类，必填
        ref={ref}
        setCheckRows={props.checkrowtwoJn}                                // 获取组件实例，选填
        formWidth={500}
