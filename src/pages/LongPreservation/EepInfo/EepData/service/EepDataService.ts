...
class EepDataService extends BaseService implements IEpsService, ITableService {

  findByKey(key: string | number, page: number = 1, size: number = 10, args: object = {}): Promise<any> {
    return new Promise((resolve, reject) => {
     return super.get({url: this.url+`/queryData/`+args.aipid})
        .then(response => {
...
export default new EepDataService('/api/api/eepinfo');
