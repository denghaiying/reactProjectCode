...
import zdgxService from './service/ZdgxService';
...
const customForm = () => {
  ...
  useEffect(() => {
    ...
    const queryzjkzdList =  async (params) =>{
          let url="/api/eps/tyjk/zdgx/queryForZjkList";
          const response =await fetch.post(url+"?id="+props.zjkid,params);
          if (response.status === 200) {
            if (response.data.length > 0) {
                 const zjkzdresponse =await fetch.post("/api/eps/tyjk/zdgx/queryForZjkZdList?zjkid="+props.zjkid+"&midtbname="+props.midtbname ,params);
    ...
    const queryjkpzList =  async () =>{
          let url="/api/eps/tyjk/jkpz/queryForId?id=";
          const response =await fetch.post(url+props.jkpzid);
          if (response.status === 200) {
            if (response.data) {
