...
import kkdyService from './service/KkdyService';
import { Form, Input, message, Select} from 'antd';
import { observer } from 'mobx-react';
import { EpsSource, ITable } from '@/eps/commons/declare';
import jkpzService from '../jkpz/service/JkpzService';
...
const tmztlist = [{value: "1", label: "文件收集"}, {value: "2", label: "文件整理"},{value: "5", label: "档案编研"}];
/**
 * 获取当前时间
 */
...
const Kkdy = observer((props) => {
  ...
  const customForm = (text, form) => {
    const jkonChange =  async (value) => {
      let url="/api/eps/tyjk/jkpz/queryForId";
        const response =await fetch.get(url+"?id="+value,{});
         if (response.status === 200) {
             var mbid=response.data.mb;
             var zjkid =response.data.zjkid;
             setMbid(mbid);
             text.setFieldsValue({ 'mb':mbid });
             text.setFieldsValue({ 'zjkid':zjkid });
             const mbresponse =await fetch.get("/api/eps/control/main/mb/queryForId?id="+mbid,{});
             if(mbresponse.status === 200){
                text.setFieldsValue({ 'mbmc': mbresponse.data.mc });
             }
        }
    }
    ...
    const dakChange =  async (value) => {
        let url="/api/eps/control/main/dak/queryForId";
          const response =await fetch.get(url+"?id="+value,{});
          if (response.status === 200) {
                text.setFieldsValue({ 'bmc': response.data.mbc });
            }
    }
    ...
    return (
      <>
        <Form.Item label="所属接口:" name="jkid" required  rules={[{ required: true, message: '请选择所属接口' }]}>
          <Select placeholder="所属接口"   options={jklist} style={{width:  _width}} onChange={jkonChange}/>
        </Form.Item>
        <Form.Item label="中间库"  name="zjkid" required  rules={[{ required: true, message: '请选择中间库' }]}>
          <Select   placeholder="中间库"  disabled  options={zjklist} style={{width:  _width}}/>
        </Form.Item>
        <Form.Item label="模版"  name="mbmc" >
  ...
  useEffect(() => {
      const queryZjkList =  async (params) =>{
        if(tableStore){
          let url="/api/eps/tyjk/kkdy/queryForZjkList";
      ...
      const queryjkList =  async (params) =>{
        if(tableStore){
          let url="/api/eps/tyjk/kkdy/queryForJkpzList";
          const response =await fetch.post(url,params);
          if (response.status === 200) {
            if (response.data.length > 0) {
      ...
      const querydakList =  async (params) =>{
        if(tableStore){
  ...
  const searchFrom = () => {
    return (
      <>
        <Form.Item label="所属接口" className="form-item" name="jkid"> <Select placeholder="所属接口"   options={jklist} /></Form.Item >
        <Form.Item label="中间库" className="form-item" name="zjkid"> <Select   placeholder="中间库"   options={zjklist} /></Form.Item >
      </>
    )
  }
