...
const ModuleAddDailog = observer(props => {
  ...
  const onModuleOk = () => {
    ...
    const treeFuncData = [];
    // 导入模块部分
    if (XtzcStore.radioChecked === 'import') {
      // 获取当前选中的模块id
      const moduleId = addForm.getFieldValue('moduleId');
      if (moduleId !== undefined) {
        XtzcStore.findFuncData(moduleId).then(() => {
          XtzcStore.isShow = false;
          if (XtzcStore.moduleData.length > 0) {
            XtzcStore.moduleData.forEach((m) => {
              if (m.id === moduleId) {
                if (treeData.length > 0) {
                  if (XtzcStore.funcData.length > 0) {
                    XtzcStore.funcData.forEach((f) => {
                      treeFuncData.push({
            ...
            // 刷新右侧模块数据及功能数据
            let moduleSelectData = [...XtzcStore.moduleSelectData];
            if (moduleSelectData.length > 0) {
              moduleSelectData = moduleSelectData.filter(f => f.value !== moduleId);
              XtzcStore.setModuleSelectData(moduleSelectData);
            ...
            // 刷新弹框导入模块数据
            let moduleSelectImportData = [...XtzcStore.moduleSelectImportData];
            if (moduleSelectImportData.length > 0) {
              moduleSelectImportData = moduleSelectImportData.filter(f => f.value !== moduleId);
              XtzcStore.setModuleSelectImportData(moduleSelectImportData);
            }
            // 关闭新增模块文件夹弹框
    ...
    } else if (XtzcStore.radioChecked === 'define') {
  ...
  const onRadioChange = (value) => {
    XtzcStore.radioChecked = value.target.value;
  }
  ...
  return (
    ...
    >
      <Form form={addForm} {...formItemLayout}>
        <Row>
          <Col>
            <Radio.Group onChange={onRadioChange} value={XtzcStore.radioChecked}>
          ...
          <Col>
            <Row>
              <Form.Item name="moduleId">
                <Select style={{ width: 250 }} allowClear options={XtzcStore.moduleSelectImportData} />
              </Form.Item>
            </Row>
            <Row>
              <Form.Item name="mkmc">
