...
const daglzxj = observer((props: DakFormProps) => {
  ...
  const store = useLocalObservable(() => ({
    ...
    async initKtable(props: DakFormProps) {
      const res = await fetch.post('/api/eps/control/main/dak/queryForId?id='+props.params.dakid);
      const ajres = await fetch.post('/api/eps/control/main/dak/queryForKey?id='+res.data.fid);
      const tmzt: string = props.params.tmzt;
      const dakid: string = ajres.data.id;
      const mbid: string = ajres.data.mbid;
      store.bmc=ajres.data.mbc;
  ...
  const onSave = () => {
    form.validateFields().then(values => {
        values.wzlids=props.extendParams?.dakGrid.ids.toString();
        const params = {
          dakid: store.dakid,
          mbid: store.mbid,
          tmzt: store.tmzt,
        ...
        TableService.save(params)
          .then((res) => {
            message.success('操作成功!');
          })
          .catch((xinfo) => {
            console.log(xinfo);
            message.error(`操作失败！原因:${xinfo}`);
          });
      })
      .catch(info => {
        console.log(info)
