...
class ResetDagsbmService extends BaseService implements IEpsStore,ITreeService,ITableService {
    ...
    findByKey(key: string | number, page: number = 1, size: number = 20, args: object = {}): Promise<any> {
      ...
      //  args['search']= key;
        ...
        savetz(data) {
            return new Promise((resolve, reject) => {
                return super.post({ url: '/api/eps/control/main/dagl/reSetDabm', params: data }).then(res => {
...
export default new ResetDagsbmService('/api/eps/control/main/org');
