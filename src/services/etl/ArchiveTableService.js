...
class ArchiveTableService extends BaseService {
  constructor(url) {
    super(url);
    this.IsLogin = false;
  ...
  findMetadata(params) {
    return new Promise((resolve, reject) => {
      fetch
          .post(this.url + "/findMetadata",params)
          .then(response => {
            if (response.status === 200) {
              resolve(response.data);
            } else {
  ...
  runTask(id) {
    return new Promise((resolve, reject) => {
      fetch
          .post(`${this.url}/runtask/${encodeURIComponent(id)}`)
          .then(response => {
...
export default new ArchiveTableService("/api/archivecolumn");
