...
class UserService extends BaseService {
  ...
  ssoLogin(loginname, password) {
    return new Promise((resolve, reject) => {
      fetch
        .get(
          `/eps9tyjk/login?loginCode=${loginname}&password=${password}&logintime=${util.getCurrentTime()}`,
          {},
        )
        .then((response) => {
          if (response.data.success) {
  ...
  checktoken(token) {
    return new Promise((resolve, reject) => {
      if (token) {
        fetch
          .get(`${this.url}/token/${encodeURIComponent(token)}`)
          .then((response) => {
            if (response.data.bh) {
...
export default new UserService('/api/eps/sso');
