...
class FlowGroupService extends BaseService implements IEpsService, ITreeService, ITableService {
  ...
  saveFzZlx(dakid: String, args: String, whr: String, whrid: String) {
    ...
    return new Promise((resolve, reject) => {
      return super.post({ url: this.url + `/saveFzZlx?dakid=${dakid}&arrs=${aa}&lx=2&whr=${whr}&whrid=${whrid}` }).then(res => {
...
export default new FlowGroupService('/api/eps/control/main/lcfz');
