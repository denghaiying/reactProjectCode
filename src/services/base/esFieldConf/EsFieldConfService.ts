...
class EsFieldConfService extends BaseService implements IEpsService, ITreeService, ITableService {
  ...
  findByKey(key: string | number, page: number = 1, size: number = 10, args: object = {}): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      return super.get({ url: this.url + '/queryForPage', params: args }).then(res => {
        ...
        return resolve(res.data.data.records)
      }).catch(err => {
        return reject(err)
      })
    }
  ...
  restoreIndex() {
    return new Promise((resolve, reject) => {
      return super.get({ url: this.url + '/restore_index' }).then(res => {
        if (res.status > 300) {
          return reject(res)
        }
        return resolve(res.data)
  ...
  searchAll() {
    return new Promise((resolve, reject) => {
      return super.get({ url: this.url + '/indices' }).then(res => {
        if (res.status > 300) {
          return reject(res)
        }
        return resolve(res.data)
  ...
  searchQwjs_index() {
    return new Promise((resolve, reject) => {
      return super.get({ url: this.url + '/search_index' }).then(res => {
        if (res.status > 300) {
          return reject(res)
        }
        return resolve(res.data)
...
export default new EsFieldConfService('/api/eps/control/main/esfiled');
