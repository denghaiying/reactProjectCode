 ...
 */
  ...
  @observable colums =[
      {
        title: 'e9.ksh.kshfl.kshflbh',
        dataIndex: 'kshflbh',
        width: 80,
      },
      {
        title:  'e9.ksh.kshfl.kshflmc',
        dataIndex: 'kshflmc',
        width: 80,
      }
      ,
      {
        title:  'e9.ksh.kshfl.kshflxh',
        dataIndex: 'kshflxh',
        width: 80,
      }
    ];
  ...
  @action setPageNo = async (pageno) => {
    ...
    this.loading = true;
    try {
        var xdataS = await kshflService.findForPage(this.params, this.pageno, this.pagesize);
        this.dataSource=[];
 ...
 @action queryData  = async () =>{
  ...
  @action queryList  = async () =>{
    try {
        ...
        var xdataS = await kshflService.findList(this.params);
        this.list=[];
        this.list=xdataS;
    } catch (err) {
      this.loading = true;
  ...
  @action saveData = async (values) => {
    try {
      if (this.opt === 'edit') {
         await kshflService.updatesome(this.editRecord.id, values);
      } else {
        await kshflService.add(values);
      }
      this.modalVisible = false;
      this.loading = true;
      var xdataS = await kshflService.findForPage(this.params, this.pageno, this.pagesize);
