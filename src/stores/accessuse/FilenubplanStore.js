...
class FilenubplanStore extends BaseStore {
    // 数据字典利用目的
    @observable mblist = [];
    @observable mbzlxlist = [];
    @action findMbkList = async () => {
        const params = {
            notree: "Y",
        };
        const list = await fetch
            .get(`eps/control/main/mb/queryForPage`, {params});
        this.mblist = list.data;
    }
    @action findZlx = async (data) => {
        const params = {
            mbid: data,
        };
        const list = await fetch
            .get(`eps/control/main/mbzlx/queryForList`, {params});
        this.mbzlxlist = list.data;
...
export default new FilenubplanStore('/api/eps/control/main/filenubplan');
