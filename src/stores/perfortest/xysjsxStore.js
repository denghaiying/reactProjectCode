...
class xysjsxStore {
  ...
  @observable deleteid = "";
  @observable colums = [{
    title: 'e9.perfortest.xysjsx.xysjsxbh',
    dataIndex: "xysjsxbh",
    width: 180
  },
  {
    title: 'e9.perfortest.xysjsx.xysjsxmc',
    dataIndex: "xysjsxmc",
    width: 300
  }];
  ...
  @action setPageNo = async (pageno) => {
    ...
    this.loading = true;
    try {
      this.dataSource = await XysjsxService.findForPage(this.params, this.pageno, this.pagesize);
      this.loading = false;
    } catch (err) {
      this.loading = false;
      throw err;
  ...
  @action queryData = async () => {
    ...
    try {
      ...
      const data = await XysjsxService.findForPage(this.params, this.pageno, this.pagesize);
  ...
  @action saveData = async (values) => {
    try {
      ...
      if (this.opt === 'edit') {
        await XysjsxService.updatesome(this.editRecord.id, values);
      } else {
        await XysjsxService.add(values);
      }
      this.modalVisible = false;
      this.loading = true;
      const data = await XysjsxService.findForPage(this.params, this.pageno, this.pagesize);
...
export default new xysjsxStore();
