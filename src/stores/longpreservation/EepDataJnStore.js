...
class EepDataJnStore extends BaseStore {
  @action async findAll (params) {
    this.loading = true;
    this.params = params;
    try {
      this.data = await EepDataService.findByEepInfoJn(this.params);
      const columns = await EepDataService.queryEepColumnJn(this.params);
...
export default new EepDataJnStore("/api/eepdata",true,false);
