...
class ZxyjStore extends BaseStore {
                    ...
                    if (resp.status === 200) {
                        ...
                        if (timer == null) {
                            timer = setInterval(() => {
                                ...
                                fetch.get(`/api/eps/control/main/gszxyjsqd/queryForId`, {params}).then(gszxyjsqd => {
    ...
    @action xduoJc = async (paramid) => {
        ...
        fetch.post(`/api/eps/control/main/gszxyjsqd/duoJc`, fd, {responseType: 'json'}).then(resp => {
            if (resp.status === 200) {
                this.progressValue = 40;
                let timer = null;
                if (timer == null) {
                    ...
                    fetch.get(`/api/eps/control/main/gszxyjsqd/queryForId`, {params}).then(gszxyjsqd => {
                       if (gszxyjsqd.data.jczt === "Y") {
                                        ...
                                        fetch.get(`/api/api/sxjcjgz/sxjcjgxq?jcbc=`+paramid, {}).then(sxjcjgxq => {
                                            ...
                                            fetch.post(`/api/eps/control/main/gszxyjsqd/uploadjg`, fd, {responseType: 'json'}).then(resp => {
                                            });
                                          //  this.editRecord = {...v, ...ywresult};
                                        });
                                    }else if (gszxyjsqd.data.jczt === "N"){
...
export default new ZxyjStore('/api/eps/control/main/gsyjsqd', true, true);
