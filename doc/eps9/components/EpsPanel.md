...
|treeService |左侧树service,用于实现和后端交互，需实现 ITreeService 接口 | [ITreeService](#itreeservice) |null | 1.0 | 
|treeProp | 左侧树配置内容，可控制左侧树的的内容 | [ITree](#itree) |  underfined | 1.0 | 
|noRender |点击左侧树是否不渲染右侧表格，如设置ture, 则开启三联模式，表格上部出现下拉树组件，左侧树选择内容触发下拉树数据变更，下拉树选择内容触发右侧表格数据 | boolean | false | 1.0 | 
|selectService |三联模式中，下拉树数据源接口，需实现 ISelectService 接口| ISelectService | null | 1.0 | 
|treeParams |左侧树初始化参数，用于左侧树加载时预处理数据 | Object | {} | 1.1 |
